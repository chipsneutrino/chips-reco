...
WCSimDigitizerLikelihood::WCSimDigitizerLikelihood() : fType(kUnknown), fSK1peFile(NULL), fSK1peHist(NULL),
													   fPMTSimFile(NULL), fPMTSimHist(NULL),
													   fTOTFile(NULL), fTOTNikhefHist(NULL), fTOTMadisonHist(NULL),
													   fEfficiency(1.0)
{
  ...
  else if( fType == WCSimDigitizerLikelihood::kTOT) { this->OpenTOTPDFs(); }
...
WCSimDigitizerLikelihood::~WCSimDigitizerLikelihood()
{
  ...
  if(fSK1peHist != NULL){ delete fSK1peHist; }
  if(fPMTSimFile != NULL){ delete fPMTSimFile; }
  if(fPMTSimHist != NULL){ delete fPMTSimFile; }
  if(fTOTFile != NULL){ delete fPMTSimFile; }
  if(fTOTNikhefHist != NULL){ delete fPMTSimFile; }
  if(fTOTMadisonHist != NULL){ delete fTOTMadisonHist; }
}
...
void WCSimDigitizerLikelihood::SetDigiType( WCSimDigitizerLikelihood::DigiType_t type )
{
  if( fType != type )
  {
    ...
    if(type == WCSimDigitizerLikelihood::kTOT){ this->OpenTOTPDFs(); }
  }
  return;
}
...
{
	if( fTOTFile != NULL) { delete fTOTFile; fTOTNikhefHist = NULL; fTOTMadisonHist = NULL;}
  ...
  switch( fType )
  {
    ...
    default:
    {
      ...
      assert(    fType == WCSimDigitizerLikelihood::kSK1pe
			  ...
			  || fType == WCSimDigitizerLikelihood::kTOT);
      break;
    }
  }
  if(m2LnL < 0 || m2LnL > 25.0){ m2LnL = 25.0; } // Implement a maximum likelihood per PMT
...
/**
 * Depending on the value of the undigitized charge, we will mimic the SK1pe digitizer in 3 separate ways
...
Double_t WCSimDigitizerLikelihood::GetSK1peMinus2LnL( const Double_t &undigi, const Double_t &digi )
{
	...
	Double_t lnL = lnL = TMath::Log( likelihood );
...
Double_t WCSimDigitizerLikelihood::GetSK1pePickerLikelihood( const Double_t &undigi, const Double_t &digi )
{
  ...
  Double_t maxQ = 20.0;
  if( undigi >= maxQ){ return this->GetSK1peGausExpoLikelihood(undigi, digi); }
...
Double_t WCSimDigitizerLikelihood::GetSK1peGausExpoLikelihood( const Double_t &undigi, const Double_t &digi)
{
 	...
 	if(undigi < minQ) { return this->GetSK1pePickerLikelihood(undigi, digi); }
 	if(undigi >= maxQ) { return this->GetSK1peGausLikelihood(undigi, digi); }
...
{
  Double_t minQ = 200.0;
  if( undigi < minQ ) return this->GetSK1peGausExpoLikelihood(undigi, digi);
...
Double_t WCSimDigitizerLikelihood::GetPMTSimMinus2LnL(const Double_t& undigi,
	...
	if(undigi >= 20 || digi >=20){ return 0; } // Just ignore PMTs outside of our know range for now...
...
Double_t WCSimDigitizerLikelihood::GetTOTMinus2LnL(const Double_t& undigi,
		const Double_t& digi, std::string PMTName) {
...
Double_t WCSimDigitizerLikelihood::GetTOTLikelihood(const Double_t& undigi,
		const Double_t& digi, std::string PMTName) {
