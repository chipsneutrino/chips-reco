...
void WCSimGeometry::PrintGeometry() {
	...
	if (WCSimGeometry::TouchGeometry()) {
		...
		if (fgGeometryHandle->GetGeoType() == WCSimGeometry::kCylinder) {
			std::cout << "  Detector Geometry: CYLINDER " << std::endl << "   radius[m]="
					<< 1.0e-2 * fgGeometryHandle->GetCylRadius() << std::endl << "   length[m]="
					<< 1.0e-2 * fgGeometryHandle->GetCylLength() << std::endl << "   fiducial_radius[m]="
					<< 1.0e-2 * fgGeometryHandle->GetCylFiducialRadius() << std::endl << "   fiducial_length[m]="
					<< 1.0e-2 * fgGeometryHandle->GetCylFiducialLength() << std::endl;
			std::cout << "   area[m^2]=" << 1.0e-4 * fgGeometryHandle->GetArea() << std::endl << "   volume[m^3]="
					<< 1.0e-6 * fgGeometryHandle->GetVolume() << std::endl << "   fiducial_volume[m^3]="
					<< 1.0e-6 * fgGeometryHandle->GetFiducialVolume() << std::endl;
		}
		if (fgGeometryHandle->GetGeoType() == WCSimGeometry::kMailBox) {
			std::cout << "  Detector Geometry: MAILBOX " << std::endl << "   dx[m]="
					<< 1.0e-2 * fgGeometryHandle->GetMailBoxX() << std::endl << "   dy[m]="
					<< 1.0e-2 * fgGeometryHandle->GetMailBoxY() << std::endl << "   dz[m]="
					<< 1.0e-2 * fgGeometryHandle->GetMailBoxZ() << std::endl << "   fiducial_dx[m]="
					<< 1.0e-2 * fgGeometryHandle->GetMailBoxFiducialX() << std::endl << "   fiducial_dy[m]="
					<< 1.0e-2 * fgGeometryHandle->GetMailBoxFiducialY() << std::endl << "   fiducial_dz[m]="
...
WCSimGeometry::WCSimGeometry() :
		fPMTs(0), fPmtX(0), fPmtY(0), fPmtZ(0), fPmtRegion(0), fWCSimRootGeom(0), fEmissionProfileManager(0) {
	this->SetGeometry(0);
}

WCSimGeometry::WCSimGeometry(WCSimRootGeom* geom) :
		fPMTs(0), fPmtX(0), fPmtY(0), fPmtZ(0), fPmtRegion(0), fWCSimRootGeom(0) {
...
Bool_t WCSimGeometry::InsideDetector(Double_t x, Double_t y, Double_t z) {
	...
	if (fGeoType == WCSimGeometry::kMailBox) {
		if (x >= -0.5 * fMailBoxX && x <= +0.5 * fMailBoxX && y >= -0.5 * fMailBoxY && y <= +0.5 * fMailBoxY
...
Bool_t WCSimGeometry::InsideFiducialVolume(Double_t x, Double_t y, Double_t z) {
	...
	if (fGeoType == WCSimGeometry::kMailBox) {
		if (x >= -0.5 * fMailBoxFiducialX && x <= +0.5 * fMailBoxFiducialX && y >= -0.5 * fMailBoxFiducialY
				&& y <= +0.5 * fMailBoxFiducialY && z >= -0.5 * fMailBoxFiducialZ && z <= +0.5 * fMailBoxFiducialZ) {
			return 1;
		}
	}
...
Double_t WCSimGeometry::DistanceToEdge(Double_t x, Double_t y, Double_t z) {
	...
	if (fGeoType == WCSimGeometry::kMailBox) {
		...
		// outside detector (convention: -ve dr)
			...
			// six edge regions
			if (x <= -0.5 * fMailBoxX && y >= -0.5 * fMailBoxY && y <= +0.5 * fMailBoxY && z >= -0.5 * fMailBoxZ
					&& z <= +0.5 * fMailBoxZ) {
			...
			if (x >= -0.5 * fMailBoxX && x <= +0.5 * fMailBoxX && y <= -0.5 * fMailBoxY && z >= -0.5 * fMailBoxZ
					&& z <= +0.5 * fMailBoxZ) {
...
void WCSimGeometry::FindCircle(Double_t x0, Double_t y0, Double_t z0, Double_t x1, Double_t y1, Double_t z1,
	...
	Double_t n[3] = { p01[1] * p12[2] - p01[2] * p12[1], p01[2] * p12[0] - p01[0] * p12[2], p01[0] * p12[1]
			- p01[1] * p12[0] };

	Double_t D2 = n[0] * n[0] + n[1] * n[1] + n[2] * n[2];

	if (D2 > 0.0) {
		Double_t a0 = -(p12[0] * p12[0] + p12[1] * p12[1] + p12[2] * p12[2])
				* (p01[0] * p20[0] + p01[1] * p20[1] + p01[2] * p20[2]) / (2.0 * D2);
		Double_t a1 = -(p20[0] * p20[0] + p20[1] * p20[1] + p20[2] * p20[2])
				* (p12[0] * p01[0] + p12[1] * p01[1] + p12[2] * p01[2]) / (2.0 * D2);
		Double_t a2 = -(p01[0] * p01[0] + p01[1] * p01[1] + p01[2] * p01[2])
				* (p20[0] * p12[0] + p20[1] * p12[1] + p20[2] * p12[2]) / (2.0 * D2);
		Double_t D = sqrt(D2);
		...
		radius = sqrt(
				(p01[0] * p01[0] + p01[1] * p01[1] + p01[2] * p01[2])
...
void WCSimGeometry::FindVertex(Double_t x0, Double_t y0, Double_t z0, Double_t t0, Double_t x1, Double_t y1,
	...
	if ((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0) + (z1 - z0) * (z1 - z0) >= c * c * (t1 - t0) * (t1 - t0)
		...
		if (!(fabs(dx1) < epsilon && fabs(dx2) < epsilon && fabs(dx3) < epsilon)
			...
			Double_t Qdata[3] = { 0.5 * (dx1 * dx1 + dy1 * dy1 + dz1 * dz1 - dt1 * dt1), 0.5
					* (dx2 * dx2 + dy2 * dy2 + dz2 * dz2 - dt2 * dt2), 0.5
					* (dx3 * dx3 + dy3 * dy3 + dz3 * dz3 - dt3 * dt3) };
