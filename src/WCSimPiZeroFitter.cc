 ...
 */
WCSimPiZeroFitter::WCSimPiZeroFitter(WCSimFitterConfig * config) :
		WCSimLikelihoodFitter(config), fPiZeroSeedGenerator(config) {
	fFitterPlots = NULL;
...
void WCSimPiZeroFitter::SetStartingTracks(WCSimPiZeroSeed * seed) {
	...
	if (CanSetParam(0, FitterParameterType::kConversionDistance)) {
		fFitterTrackParMap.SetCurrentValue(0, FitterParameterType::kConversionDistance,
				seed->GetTrack1()->GetConversionDistance());
	}
	...
	if (CanSetParam(1, FitterParameterType::kConversionDistance)) {
		fFitterTrackParMap.SetCurrentValue(1, FitterParameterType::kConversionDistance,
				seed->GetTrack2()->GetConversionDistance());
	}
...
void WCSimPiZeroFitter::FitEventNumber(Int_t iEvent) {
	...
	while (seedTrackItr != allSeedTracks.end()) {
		...
		fSeeds.push_back(
				WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, seed->GetTrack1()->GetX(),
						seed->GetTrack1()->GetY(), seed->GetTrack1()->GetZ(), seed->GetTrack1()->GetT(),
						seed->GetTrack1()->GetTheta(), seed->GetTrack1()->GetPhi(), seed->GetTrack1()->GetE(),
						seed->GetTrack1()->GetConversionDistance()));

		fSeeds.push_back(
				WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, seed->GetTrack2()->GetX(),
						seed->GetTrack2()->GetY(), seed->GetTrack2()->GetZ(), seed->GetTrack2()->GetT(),
						seed->GetTrack2()->GetTheta(), seed->GetTrack2()->GetPhi(), seed->GetTrack2()->GetE(),
						seed->GetTrack2()->GetConversionDistance()));
...
std::vector<std::pair<WCSimLikelihoodTrackBase *, WCSimLikelihoodTrackBase*> > WCSimPiZeroFitter::SeedCheat() {
	...
	WCSimLikelihoodTrackBase * track1 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ,
			vtxT, 2.3085, 0.5737, 541.052, map50);
	WCSimLikelihoodTrackBase * track2 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ,
			vtxT, 1.9172, 0.5348, 221.180, map50);
	allSeeds.push_back(std::make_pair(track1, track2));

	track1 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ, vtxT, 2.3085, 0.5737,
			528.678, map250);
	track2 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ, vtxT, 1.9955, 0.5412,
			352.048, map250);
	allSeeds.push_back(std::make_pair(track1, track2));
	...
	track1 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ, vtxT, 2.3085, 0.5737,
			541.052, map250);
	track2 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike, vtxX, vtxY, vtxZ, vtxT, 1.9172, 0.5348,
			221.180, map250);
