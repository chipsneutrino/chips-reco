...
void WCSimDataCleaner::RunPrintParameters() {
	...
	std::cout << "  Data Cleaner Parameters: " << std::endl << "   Config = " << fConfig << std::endl
			<< "   MinPulseHeight = " << fMinPulseHeight << std::endl << "   NeighbourRadius = " << fNeighbourRadius
			<< std::endl << "   MinNeighbourDigits = " << fMinNeighbourDigits << std::endl << "   ClusterRadius = "
			<< fClusterRadius << std::endl << "   MinClusterDigits = " << fMinClusterDigits << std::endl
...
std::vector<WCSimRecoDigit*>* WCSimDataCleaner::Run(std::vector<WCSimRecoDigit*>* myDigitList) {
	...
	if (fConfig == WCSimDataCleaner::kNone)
		return myOutputList;
	...
	myOutputList = FilterDigits(myOutputList);
	if (fConfig == WCSimDataCleaner::kPulseHeight)
		return myOutputList;
	...
	myOutputList = FilterDigits(myOutputList);
	if (fConfig == WCSimDataCleaner::kPulseHeightAndNeighbours)
		return myOutputList;
	...
	myOutputList = FilterDigits(myOutputList);
	if (fConfig == WCSimDataCleaner::kPulseHeightAndClusters)
		return myOutputList;
...
std::vector<WCSimRecoDigit*>* WCSimDataCleaner::FilterByNeighbours(std::vector<WCSimRecoDigit*>* myDigitList) {
	...
	for (UInt_t idigit1 = 0; idigit1 < myDigitList->size(); idigit1++) {
		for (UInt_t idigit2 = idigit1 + 1; idigit2 < myDigitList->size(); idigit2++) {
			...
			if (drsq > 0.0 && drsq < fNeighbourRadius * fNeighbourRadius && fabs(dt) < fTimeWindow) {
...
std::vector<WCSimRecoCluster*>* WCSimDataCleaner::RecoClusters(std::vector<WCSimRecoDigit*>* myDigitList) {
	...
	for (UInt_t idigit1 = 0; idigit1 < vClusterDigitList.size(); idigit1++) {
		for (UInt_t idigit2 = idigit1 + 1; idigit2 < vClusterDigitList.size(); idigit2++) {
			...
			if (drsq > 0.0 && drsq < fClusterRadius * fClusterRadius && fabs(dt) < fTimeWindow) {
				...
				for (UInt_t jdigit = 0; jdigit < vClusterDigitCollection.size(); jdigit++) {
					...
					if (cdigit->IsAllClustered() == 0) {
						for (Int_t kdigit = 0; kdigit < cdigit->GetNClusterDigits(); kdigit++) {
							WCSimRecoClusterDigit* cdigitnew =
									(WCSimRecoClusterDigit*) (cdigit->GetClusterDigit(kdigit));
