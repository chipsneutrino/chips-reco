...
//	}
	...
	for(unsigned int se = 0; se < slicedEvents.size(); ++se)
	{
	  ...
	  {
		  std::pair<WCSimRecoRing*,double> tempPair;
		  tempPair = std::make_pair<WCSimRecoRing*,double>(slicedEvents[se]->GetRing(r),slicedEvents[se]->GetVtxTime());
	...
	for(int i = 0; i <= 1; ++i)
	{
		...
		if( ringList.size() >= 2 )
		{
			...
			WCSimLikelihoodTrackBase * track1 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike,
			...
			WCSimLikelihoodTrackBase * track2 = WCSimLikelihoodTrackFactory::MakeTrack(TrackType::PhotonLike,
																					   seedX, seedY, seedZ, seedT,
																					   seedTheta, seedPhi,
																					   seedE,
																					   0.0);
...
void WCSimPiZeroHoughSeeder::SetStartingEnergies(WCSimRecoRing* ring1,
		WCSimRecoRing* ring2, WCSimLikelihoodTrackBase* track1,
		WCSimLikelihoodTrackBase* track2)
{
		...
		double &convDistance2)
{
	...
	if(vtxDiff.Mag() < 5) { return track1->GetVtx(); }
