...
WCSimIntegralLookupMaker3D::WCSimIntegralLookupMaker3D(TrackType::Type particle, int nR0Bins, double R0Min,
		double R0Max, int nCosTh0Bins, double cosTh0Min, double cosTh0Max) {
	std::cout << " *** WCSimIntegralLookupMaker3D for track type = " << TrackType::AsString(particle) << " *** "
			<< std::endl;
	fType = particle;
...
void WCSimIntegralLookupMaker3D::SetBins(const int &nEBins, const double &eMin, const double &eMax, const int &nSBins,
	...
	fIntegrals = WCSimIntegralLookupHistArray(fEMin, fEMax, fR0Max, fR0Min, fNR0Bins, fCosTh0Max, fCosTh0Min,
			fNCosTh0Bins);
}
...
void WCSimIntegralLookupMaker3D::MakeRhoGSplines() {
	...
	for (int iR0Bin = 0; iR0Bin < fNR0Bins; ++iR0Bin) {
		...
		for (int iCosTh0Bin = 0; iCosTh0Bin < fNCosTh0Bins; ++iCosTh0Bin) {
			...
			if (R0 == 1700 && cosTh0 == 0.96) {
				std::cout << "RhoGSSInt = " << grSS << std::endl;
			}
