	...
	if (!isBarrelSeparate) {
		...
		if (mergeTopAndUpperSide && mergeBottomAndLowerSide) {
			...
			if (isBottomHit)
				...
				lowerHit = fVetoDigits->at(indexMaxSideLower);
		} else if (!mergeTopAndUpperSide && mergeBottomAndLowerSide) {
			int lowerIndex = indexMaxSideLower;
			if (qMaxBottom > qMaxSideLower) {
				lowerIndex = indexMaxBottom;
			...
			if (isTopHit)
				...
				upperHit = fVetoDigits->at(indexMaxSideUpper);
			lowerHit = fVetoDigits->at(lowerIndex);
		} else {
			// No merging...
...
void WCSimCosmicSeed::RunClusteringSeed() {
	...
	if (vetoClusters.size() < 2) {
		while ((vetoClusters.size() < 2) && (chargeCut < (maxCharge - 0.99))) {
			++chargeCut;
			std::cout << "WCSimCosmicSeed: Could not form two clusters. Raising charge cut to " << chargeCut
					<< std::endl;
...
void WCSimCosmicSeed::GetSimpleDirectionFromInner() {
	...
	for (unsigned int i = 0; i < fInnerDigits->size(); ++i) {
		...
		if (digit->GetRegion() != 0)
			continue; // Top
		if (digit->GetRegion() == 1 && digit->GetX() > 0)
			continue; // Front barrel
...
void WCSimCosmicSeed::GenerateRingPoints() {
	...
	myGeo->ProjectToFarEdge(fFittedVtx.X(), fFittedVtx.Y(), fFittedVtx.Z(), fFittedDir.X(), fFittedDir.Y(),
			fFittedDir.Z(), xproj, yproj, zproj, region);
...
//  std::cout << "x/D:y:z" << std::endl;
	for (Int_t n = 0; n < nPoints; n++) {
		...
		myGeo->FindCircle(xproj, yproj, zproj, fFittedVtx.X(), fFittedVtx.Y(), fFittedVtx.Z(), cone_angle,
				n * delta_omega, xring, yring, zring, nx, ny, nz, radius);
