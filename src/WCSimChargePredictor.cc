...
double WCSimChargePredictor::GetPredictedCharge(WCSimLikelihoodDigit *myDigit) {
	...
	if (predictedCharge < 1e-6) {
		predictedCharge = 1e-6;
	}
...
double WCSimChargePredictor::ChargeExpectation(Int_t trackIndex) {
	...
	if (muDir < 0) {
		std::cerr << "Warning: predicted direct charge was less than zero (" << muDir << ") - returning zero"
				<< std::endl;
		muDir = 0;
	}
	if (muIndir < 0) {
...
double WCSimChargePredictor::ChargeExpectation(Int_t trackIndex, WCSimLikelihoodDigit * myDigit) {
	...
	fDigit = myDigit;
	if (fGotTrackParameters != trackIndex)
		this->GetTrackParameters(trackIndex);
...
double WCSimChargePredictor::GetMuDirect(Int_t trackIndex) {
	...
	if (fGotTrackParameters == trackIndex) {
		...
		std::vector < Double_t > integrals = fTuner->GetChIntegrals(fTracks[trackIndex], fDigit);
		...
		muDirect = lightFlux
				* (integrals[0] * fCoeffsCh[0] + integrals[1] * fCoeffsCh[1] + integrals[2] * fCoeffsCh[2]);
...
double WCSimChargePredictor::GetMuIndirect(Int_t trackIndex) {
	...
	if (fGotTrackParameters == trackIndex) //FIXME: no longer necessary?
		...
		muIndirect = lightFlux
				* (integrals[0] * fCoeffsInd[0] + integrals[1] * fCoeffsInd[1] + integrals[2] * fCoeffsInd[2]);
//    if(muIndirect < 0.01)
//    {
...
void WCSimChargePredictor::GetTrackParameters(Int_t trackIndex) {
	...
	std::vector < Double_t > coeffs = fTuner->CalculateCoefficientsVector(fTracks[trackIndex], fDigit);
