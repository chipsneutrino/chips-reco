...
WCSimRecoClusteringUtil::WCSimRecoClusteringUtil(std::vector<WCSimRecoDigit*> digitVec, double qCut, double tCut, double dCut, unsigned int minSize){
...
void WCSimRecoClusteringUtil::SliceByTime(){
  ...
  if(fChargeCutDigits.size() == 0) return;
...
void WCSimRecoClusteringUtil::SliceInSpace(){
  ...
  if(fTimeSlicedDigits.size() == 0) return;
  ...
  //            << " to " << digitVector[digitVector.size()-1]->GetRawQPEs() << " PE." << std::endl;
    ...
    for(unsigned int v = 0; v < slicedDigits.size(); ++v){
      if(slicedDigits[v].size() >= fMinSliceSize){
        ...
        std::cout << "Found cluster in time slice " << t << " with " << slicedDigits[v].size() << " hits." << std::endl;

        // For each slice, make a TH3D we can look at.
        std::stringstream plotName;
        plotName << "slice_" << (t*10)+v;
        TH3D *hHist = new TH3D(plotName.str().c_str(),"",150,-1500,1500,150,-1500,1500,150,-1500,1500);
