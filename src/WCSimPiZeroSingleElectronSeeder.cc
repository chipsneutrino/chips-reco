...
WCSimPiZeroSingleElectronSeeder::WCSimPiZeroSingleElectronSeeder(WCSimFitterConfig * config) : WCSimPiZeroSeeder(config)
{
...
{
	if(fSingleElectronSeed != 0x0) { delete fSingleElectronSeed;}
}
...
void WCSimPiZeroSingleElectronSeeder::MakeSeeds()
{
	...
	fSingleElectronSeed = WCSimLikelihoodTrackFactory::MakeTrack(
								fFitterTrackParMap.GetTrackType(0),
