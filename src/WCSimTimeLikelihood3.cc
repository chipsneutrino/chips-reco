...
double LogFinder::LogFirstArrival(const double x) const {
	...
	return (log(fN * TMath::Sqrt2() / (fPredRMS * WCSimTimeLikelihood3::fSqrtPi)) - fN * WCSimTimeLikelihood3::fLog2
...
double LogFinder::LogFirstArrivalGradient(const double x) const {
	double xmm = x - fPredMean;
	return 1.0 / (fPredRMS * TMath::Sqrt2() * WCSimTimeLikelihood3::fSqrtPi) * 2
			* TMath::Exp(-xmm * xmm / (2 * fPredRMS * fPredRMS))
			* ((1 - fN) / TMath::Erfc(xmm / (fPredRMS * TMath::Sqrt2()))) - (xmm) / (fPredRMS * fPredRMS);
}
...
std::vector<std::vector<double> > WCSimTimeLikelihood3::Calc2LnLComponentsAllDigits() {
	std::vector < std::vector<double>
			> minus2LnLs(fLikelihoodDigitArray->GetNDigits(), std::vector<double>(fTracks.size()));
...
double WCSimTimeLikelihood3::CalcDigit2LnL(const unsigned int& iDigit, WCSimLikelihoodTrackBase* myTrack) {
	...
	if (IsGoodDigit(myDigit)) {
		...
		if (whichCase == 0x11) // Predict and detect a hit - use PDF overlap
			...
			timeLikelihood = (1 - scatterFrac)
					* FindOverlap(nPhotons, prediction.GetMean(), prediction.GetRMS(), myDigit->GetT(), reso);
			...
			if (timeLikelihood <= fMinimumLikelihood || timeLikelihood > 1) {
				...
				timeLikelihood = (1 - scatterFrac)
						* ig.Integral(prediction.GetMean() - 5 * prediction.GetRMS(),
	...
	for (size_t iEnergy = 0; iEnergy < fSVec.size(); ++iEnergy) {
		...
		for (std::vector<StepParameters>::iterator stepItr = stepParameterVec.begin();
			...
			if (prob > 0) {
				...
				t = myTrack->GetT() + fSVec[iEnergy]->GetBinCenter(stepItr->GetSBin()) / fSpeedOfParticle
				...
				uncertaintySquared = (0.02 * particleDistance / fSpeedOfParticle)
						* (0.02 * particleDistance / fSpeedOfParticle)
						+ (0.02 * photonDistance * n / speedOfLightInCmPerNs)
								* (0.02 * photonDistance * n / speedOfLightInCmPerNs);
			} else {
				prob = 0;
			}
...
double WCSimTimeLikelihood3::IntegrateMeanTimeToMeanFirstTime(const double &t, const double &mean, const double &sigma,
	...
	if (nPhotons <= 25) {
		integral = 1.0
				- 1.0 / (pow(2, nPhotons))
						* pow((1.0 - WCSimFastMath::erf((t - mean) / (TMath::Sqrt2() * sigma))), nPhotons);
	} else {
		double exponent = nPhotons
				* (log(1 - WCSimFastMath::erf((t - mean) / (TMath::Sqrt2() * sigma))) - WCSimTimeLikelihood3::fLog2);
...
double WCSimTimeLikelihood3::MinimumOfArrivalAndResolution(const double &t, const double &meanArr, const double rmsArr,
	...
	double resolutionPDF = 1.0 / (pmtRes * root2pi)
			* TMath::Exp(-(t - pmtTime) * (t - pmtTime) / (2 * pmtRes * pmtRes));
