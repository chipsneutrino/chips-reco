...
// Constructor
///////////////////////////////////////////////////////////////////////////
WCSimTotalLikelihood::WCSimTotalLikelihood(WCSimLikelihoodDigitArray *myLikelihoodDigitArray) : fLikelihoodDigitArray(myLikelihoodDigitArray), fDigitizerLikelihood()
{
...
Double_t WCSimTotalLikelihood::Calc2LnL()
{
	...
	for (int iDigit = 0; iDigit < fLikelihoodDigitArray->GetNDigits(); ++iDigit)
	{
		...
		{
			std::cerr << "iDigit = " << iDigit << ", and charge = " << charge2LnL << " and time = " << time2LnL
					  << std::endl;
...
void WCSimTotalLikelihood::CalcChargeLikelihoods(const std::vector<std::vector<double>> &chargePredictions)
{
	...
	for (int iDigit = 0; iDigit < fLikelihoodDigitArray->GetNDigits(); ++iDigit)
	{
		...
		if (WCSimParameters::Instance()->UseCharge())
		{
...
void WCSimTotalLikelihood::CalcTimeLikelihoods(const std::vector<std::vector<double>> &chargePredictions)
{
	...
	// =================================
	if (WCSimParameters::Instance()->UseTime())
	{
			...
			for (int iDigit = 0; iDigit < fLikelihoodDigitArray->GetNDigits(); ++iDigit)
			{
				...
				if (total > 0)
				{
					...
					if (TMath::IsNaN(timePart))
					{
						...
						{
							double trackWeight = chargePredictions[iDigit][iTrack] / total;
							std::cout << "  track " << iTrack << " weight = " << chargePredictions[iDigit][iTrack]
									  << " / " << total << " and -2LnL = " << timeParts[iDigit][iTrack] << " so prob = "
									  << exp(-0.5 * timeParts[iDigit][iTrack]) * trackWeight << std::endl;
						}
				...
				if (fTime2LnL[iDigit] < 0)
				{
					...
					{
						std::cerr << "Warning: negative -2LnL of " << fTime2LnL[iDigit] << " for digit " << iDigit
...
void WCSimTotalLikelihood::SetLikelihoodDigitArray(WCSimLikelihoodDigitArray *likelihoodDigitArray)
{
...
std::vector<double> WCSimTotalLikelihood::GetMeasuredChargeVector() const
{
	if (!fSetVectors)
	{
		...
		std::cerr << "       Have you reset the tracks or likelihood digit array since calculating the likelihood?"
				  << std::endl;
		assert(fSetVectors);
	}
...
std::vector<double> WCSimTotalLikelihood::GetPredictedTimeVector() const
{
	...
	{
		std::cerr
			<< "You're asking for the predicted time vector, but the time component of the likelihood is switched off"
			<< std::endl;
...
double WCSimTotalLikelihood::SumTotalCharges(const std::vector<double> &totalCharges) const
{
	...
	if (totalCharge < WCSimChargePredictor::GetMinAllowed())
	{
		totalCharge = WCSimChargePredictor::GetMinAllowed();
	}
	return totalCharge;
}
