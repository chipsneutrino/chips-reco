...
EventHeader::EventHeader() : fUID(""), fInputFile(""), fInputEvent(-999), fFailed(false), fRecoType(
																							  "")
{
	// Empty
}
...
EventHeader::EventHeader(std::string inputFile, int inputEvent, bool failed,
						 std::string recoType) : fInputFile(inputFile), fInputEvent(inputEvent), fFailed(failed), fRecoType(recoType)
{
	BuildUID();
}

EventHeader::EventHeader(const EventHeader &other) : fUID(other.fUID), fInputFile(other.fInputFile), fInputEvent(
																										 other.fInputEvent),
													 fFailed(other.fFailed), fRecoType(
																				 other.fRecoType)
{
	// Empty
}
...
{
	std::cout << "ParameterInfo::Print()..." << std::endl
			  << "SlicerClusterDistance -> " << fSlicerClusterDistance
			  << std::endl
			  << "SlicerMinSize -> " << fSlicerMinSize << std::endl
			  << "SlicerChargeCut -> "
			  << fSlicerChargeCut << std::endl
			  << "SlicerTimeCut -> " << fSlicerTimeCut << std::endl
			  << "IterateSlicing -> " << fIterateSlicing << std::endl
			  << "VetoClusterDistance -> " << fVetoClusterDistance
			  << std::endl
			  << "VetoMinSize -> " << fVetoMinSize << std::endl
			  << "VetoMinChargeCut -> "
			  << fVetoMinChargeCut << std::endl
			  << "VetoMaxChargeCut -> " << fVetoMaxChargeCut << std::endl
			  << "VetoTimeCut -> " << fVetoTimeCut << std::endl
			  << "CalculateIntegrals -> " << fCalculateIntegrals
			  << std::endl
			  << "TruncateIntegrals -> " << fTruncateIntegrals << std::endl
			  << "ConstrainExtent -> "
			  << fConstrainExtent << std::endl
			  << "UseTransmission -> " << fUseTransmission << std::endl
			  << "UseAngularEfficiency -> " << fUseAngularEfficiency << std::endl
			  << "UseGlassCathodeReflection -> "
			  << fUseGlassCathodeReflection << std::endl
			  << "UseScatteringTable -> " << fUseScatteringTable << std::endl
			  << "UseNewAngularEfficiency -> " << fUseNewAngularEfficiency << std::endl
			  << "UseTrackFit -> " << fUseTrackFit << std::endl
			  << "UseTime -> " << fUseTime << std::endl
			  << "UseCharge -> " << fUseCharge << std::endl
			  << "EqualiseChargeAndTime -> " << fEqualiseChargeAndTime << std::endl
			  << "SaveWCSimRootEvent -> "
			  << fSaveWCSimRootEvent << std::endl
			  << "DigiType -> " << fDigiType << std::endl
			  << "SaveSeedInfo -> "
			  << fSaveSeedInfo << std::endl
			  << "SaveStageInfo -> " << fSaveStageInfo << std::endl
			  << "SaveHitComparison -> " << fSaveHitComparison << std::endl
			  << "SaveParameters -> " << fSaveParameters << std::endl
			  << "UseCustomParticleSpeed -> " << fUseCustomParticleSpeed << std::endl
			  << "UseCustomSpeedOfLight -> "
			  << fUseCustomSpeedOfLight << std::endl
			  << "UseFittedSpeedOfLight -> " << fUseFittedSpeedOfLight << std::endl
			  << "CustomParticleSpeed -> " << fCustomParticleSpeed << std::endl
			  << "CustomSpeedOfLight -> "
			  << fCustomSpeedOfLight << std::endl
			  << "FittedSpeedOfLight -> " << fFittedSpeedOfLight << std::endl
			  << "UseSimpleTimeResolution -> " << fUseSimpleTimeResolution << std::endl
			  << "UseSimpleTimeSlew -> "
			  << fUseSimpleTimeSlew << std::endl
			  << "UseSimpleRefractiveIndex -> " << fUseSimpleRefractiveIndex
			  << std::endl;
	return;
}
...
TruthInfo::TruthInfo() : fType(-999), fBeamPDG(-999), fBeamEnergy(0.0), fNPrimaries(-999), fIsCC(
																							   false),
						 fIsNC(false), fIsRes(false), fIsDIS(false), fIsCoherent(
																		 false),
						 fIsNueElectronElastic(false), fIsInverseMuonDecay(
														   false),
						 fIsOther(false), fVtxTime(0.0), fVtxX(0.0), fVtxY(0.0), fVtxZ(
																					 0.0),
						 fBeamDirX(0.0), fBeamDirY(0.0), fBeamDirZ(0.0)
{
	fPrimaryPDGs.clear();
...
TruthInfo::TruthInfo(int type, int beamPDG, float beamEnergy, int nPrimaries,
					 ...
					 std::vector<TVector3> primaryDirs) : fType(type), fBeamPDG(beamPDG), fBeamEnergy(beamEnergy), fNPrimaries(nPrimaries), fPrimaryPDGs(primaryPDGs), fPrimaryEnergies(primaryEnergies), fPrimaryDirs(primaryDirs)
{
...
TruthInfo::TruthInfo(const TruthInfo &other) : fType(other.fType), fBeamPDG(other.fBeamPDG), fBeamEnergy(
																								 other.fBeamEnergy),
											   fNPrimaries(other.fNPrimaries), fPrimaryPDGs(
																				   other.fPrimaryPDGs),
											   fPrimaryEnergies(other.fPrimaryEnergies), fPrimaryDirs(
																							 other.fPrimaryDirs),
											   fIsCC(other.fIsCC), fIsNC(other.fIsNC), fIsQE(
																						   other.fIsQE),
											   fIsRes(other.fIsRes), fIsDIS(other.fIsDIS), fIsCoherent(
																							   other.fIsCoherent),
											   fIsNueElectronElastic(
												   other.fIsNueElectronElastic),
											   fIsInverseMuonDecay(
												   other.fIsInverseMuonDecay),
											   fIsOther(other.fIsOther), fVtxTime(
																			 other.fVtxTime),
											   fVtxX(other.fVtxX), fVtxY(other.fVtxY), fVtxZ(
																						   other.fVtxZ),
											   fBeamDirX(other.fBeamDirX), fBeamDirY(
																			   other.fBeamDirY),
...
PidInfo::PidInfo() : fVeto(false), fNVetoHits(0), fNHits(0), fNHitsUpstream(0), fNHitsDownstream(
																					0),
					 fNHitsInBottom(0), fNHitsInTop(0), fNHitsAboveMid(0), fNHitsBelowMid(
																			   0),
					 fFracHitsUpstream(0.0), fFracHitsDownstream(0.0), fFracHitsInBottom(
																		   0.0),
					 fFracHitsInTop(0.0), fFracHitsAboveMid(0.0), fFracHitsBelowMid(
																	  0.0),
					 fTotalQ(0.0), fTotalQUpstream(0.0), fTotalQDownstream(
															 0.0),
					 fTotalQInBottom(0.0), fTotalQInTop(0.0), fTotalQAboveMid(
																  0.0),
					 fTotalQBelowMid(0.0), fFracQUpstream(0.0), fFracQDownstream(
																	0.0),
					 fFracQInBottom(0.0), fFracQInTop(0.0), fFracQAboveMid(
																0.0),
					 fFracQBelowMid(0.0), fTotal2LnL(0.0), fHit2LnL(0.0), fCharge2LnL(
																			  0.0),
					 fTime2LnL(0.0), fCutoff2LnL(0.0), fTotalQInRing(0.0), fTotalQOutsideRing(
																			   0.0),
					 fTotalQInRingHole(0.0), fNHitsInRing(0), fNHitsOutsideRing(
																  0),
					 fNHitsInRingHole(0), fPredQInRing(0.0), fPredQOutsideRing(
																 0.0),
					 fPredQInRingHole(0.0), fFracTotalQInRing(0.0), fFracTotalQOutsideRing(
																		0.0),
					 fFracTotalQInRingHole(0.0), fFracNHitsInRing(0.0), fFracNHitsOutsideRing(
																			0.0),
					 fFracNHitsInRingHole(0.0), fFracPredQInRing(0.0), fFracPredQOutsideRing(
																		   0.0),
					 fFracPredQInRingHole(0.0), fPredictedOverTotalCharge(0.0), fVtxTime(
																					0.0),
					 fEnergy(0.0), fVtxX(0.0), fVtxY(0.0), fVtxZ(0.0), fVtxRho(
																		   0.0),
					 fEndX(0.0), fEndY(0.0), fEndZ(0.0), fEndRho(0.0), fDirX(
																		   0.0),
					 fDirY(0.0), fDirZ(0.0), fEscapes(false)
{
	// Empty
}

PidInfo::PidInfo(const PidInfo &other) : fVeto(other.fVeto), fNVetoHits(other.fNVetoHits), fNHits(other.fNHits), fNHitsUpstream(
																													 other.fNHitsUpstream),
										 fNHitsDownstream(other.fNHitsDownstream), fNHitsInBottom(
																					   other.fNHitsInBottom),
										 fNHitsInTop(other.fNHitsInTop), fNHitsAboveMid(
																			 other.fNHitsAboveMid),
										 fNHitsBelowMid(other.fNHitsBelowMid), fFracHitsUpstream(
																				   other.fFracHitsUpstream),
										 fFracHitsDownstream(
											 other.fFracHitsDownstream),
										 fFracHitsInBottom(
											 other.fFracHitsInBottom),
										 fFracHitsInTop(other.fFracHitsInTop), fFracHitsAboveMid(
																				   other.fFracHitsAboveMid),
										 fFracHitsBelowMid(
											 other.fFracHitsBelowMid),
										 fTotalQ(other.fTotalQ), fTotalQUpstream(
																	 other.fTotalQUpstream),
										 fTotalQDownstream(
											 other.fTotalQDownstream),
										 fTotalQInBottom(
											 other.fTotalQInBottom),
										 fTotalQInTop(other.fTotalQInTop), fTotalQAboveMid(
																			   other.fTotalQAboveMid),
										 fTotalQBelowMid(other.fTotalQBelowMid), fFracQUpstream(
																					 other.fFracQUpstream),
										 fFracQDownstream(other.fFracQDownstream), fFracQInBottom(
																					   other.fFracQInBottom),
										 fFracQInTop(other.fFracQInTop), fFracQAboveMid(
																			 other.fFracQAboveMid),
										 fFracQBelowMid(other.fFracQBelowMid), fTotal2LnL(
																				   other.fTotal2LnL),
										 fHit2LnL(other.fHit2LnL), fCharge2LnL(
																	   other.fCharge2LnL),
										 fTime2LnL(other.fTime2LnL), fCutoff2LnL(
																		 other.fCutoff2LnL),
										 fTotalQInRing(other.fTotalQInRing), fTotalQOutsideRing(
																				 other.fTotalQOutsideRing),
										 fTotalQInRingHole(
											 other.fTotalQInRingHole),
										 fNHitsInRing(other.fNHitsInRing), fNHitsOutsideRing(
																			   other.fNHitsOutsideRing),
										 fNHitsInRingHole(
											 other.fNHitsInRingHole),
										 fPredQInRing(other.fPredQInRing), fPredQOutsideRing(
																			   other.fPredQOutsideRing),
										 fPredQInRingHole(
											 other.fPredQInRingHole),
										 fFracTotalQInRing(
											 other.fFracTotalQInRing),
										 fFracTotalQOutsideRing(
											 other.fFracTotalQOutsideRing),
										 fFracTotalQInRingHole(
											 other.fFracTotalQInRingHole),
										 fFracNHitsInRing(
											 other.fFracNHitsInRing),
										 fFracNHitsOutsideRing(
											 other.fFracNHitsOutsideRing),
										 fFracNHitsInRingHole(
											 other.fFracNHitsInRingHole),
										 fFracPredQInRing(
											 other.fFracPredQInRing),
										 fFracPredQOutsideRing(
											 other.fFracPredQOutsideRing),
										 fFracPredQInRingHole(
											 other.fFracPredQInRingHole),
										 fPredictedOverTotalCharge(
											 other.fPredictedOverTotalCharge),
										 fVtxTime(other.fVtxTime), fEnergy(
																	   other.fEnergy),
										 fVtxX(other.fVtxX), fVtxY(other.fVtxY), fVtxZ(
																					 other.fVtxZ),
										 fVtxRho(other.fVtxRho), fEndX(other.fEndX), fEndY(
																						 other.fEndY),
										 fEndZ(other.fEndZ), fEndRho(other.fEndRho), fDirX(
																						 other.fDirX),
										 fDirY(other.fDirY), fDirZ(other.fDirZ), fEscapes(
																					 other.fEscapes)
{
	// Empty
}
...
void PidInfo::Print()
{
	std::cout << "HitInfo::Print()..." << std::endl
			  ...
			  << "NHitsUpstream -> "
			  << fNHitsUpstream << std::endl
			  << "NHitsDownstream -> "
			  << fNHitsDownstream << std::endl
			  << "NHitsInBottom -> "
			  << fNHitsInBottom << std::endl
			  << "NHitsInTop -> " << fNHitsInTop
			  << std::endl
			  ...
			  << "TotalQUpstream -> "
			  << fTotalQUpstream << std::endl
			  << "TotalQDownstream -> "
			  << fTotalQDownstream << std::endl
			  << "TotalQInBottom -> "
			  << fTotalQInBottom << std::endl
			  << "TotalQInTop -> " << fTotalQInTop
			  << std::endl
			  << "fTotalQAboveMid -> " << fTotalQAboveMid
			  << std::endl
			  << "fTotalQBelowMid -> " << fTotalQBelowMid
			  << std::endl
...
SeedInfo::SeedInfo() : fNSlices(-999)
{
	fTracks.clear();
	fRingHeight.clear();
	fRingTime.clear();
...
SeedInfo::SeedInfo(std::vector<WCSimLikelihoodTrackBase *> tracks, int slices,
				   std::vector<WCSimRecoRing *> ringVec, std::vector<double> ringTime) : fTracks(tracks), fNSlices(slices), fRingTime(ringTime)
{
	fRingHeight.clear();
	fRingAngle.clear();
	fRingVtx.clear();
...
SeedInfo::SeedInfo(const SeedInfo &other) : fTracks(other.fTracks), fNSlices(other.fNSlices), fRingHeight(
																								  other.fRingHeight),
											fRingTime(other.fRingTime), fRingAngle(
																			other.fRingAngle),
											fRingVtx(other.fRingVtx), fRingDir(
																		  other.fRingDir)
{
	// Empty...
}
...
StageInfo::StageInfo(const StageInfo &other) : fStageNcalls(other.fStageNcalls), fStagePreds(other.fStagePreds), fStageTracks(
																													 other.fStageTracks)
{
	// Empty
}
...
WCSimOutputTree::WCSimOutputTree() : fSaveFile(0x0), fResultsTree(0x0), fGeoTree(0x0), fGeometry(0x0), fParameterTree(0x0),
										...
										WCSimFitterConfig *fitterConfig)
{
	...
	else
	{
		...
		TString saveNameTree = Form("fit_%s_%04d_to_%04d_%s_tree.root",
									...
									fitterConfig->GetNumEventsToFit() + fitterConfig->GetFirstEventToFit(), time.Data());
