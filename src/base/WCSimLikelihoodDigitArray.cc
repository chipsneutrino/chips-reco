...
WCSimLikelihoodDigitArray::WCSimLikelihoodDigitArray(WCSimRootEvent *myEvent)
{
	...
	WCSimRecoSeed *myReco = new WCSimRecoSeed();
	WCSimRecoEvent *myRecoEvent = WCSimInterface::RecoEvent();
	myReco->RunFilter(myRecoEvent);
	...
	while (filterIter != filteredDigits->end())
	{
		...
		new (digitArray[arrayIndex]) WCSimLikelihoodDigit(pos[0], pos[1], pos[2], t, q, tubeID, face[0], face[1],
														  face[2], name, WCSimDetectorParameters::WavelengthAveragedQE(name.Data()),
														  WCSimDetectorParameters::QEAveragedRefIndex(name.Data()),
														  WCSimDetectorParameters::PMTExposeHeight(name.Data()));
		fNumHitPMTs++;
		filterIter++;

		if (t < fFirstTime || fFirstTime == -999)
		{
			fFirstTime = t;
		}
		if (t > fLastTime || fLastTime == -999)
		{
			fLastTime = t;
		}
	}
...
WCSimLikelihoodDigitArray::WCSimLikelihoodDigitArray(WCSimRootEvent *myRootEvent, Bool_t useUndigitized)
{
	...
	while (WCSimRootCherenkovHit *myChHit = (WCSimRootCherenkovHit *)undigiHitItr.Next())
	{
		...
		new (digitArray[arrayIndex]) WCSimLikelihoodDigit(posX, posY, posZ, t, Q, tubeID, faceX, faceY, faceZ, name,
														  WCSimDetectorParameters::WavelengthAveragedQE(name.Data()),
														  WCSimDetectorParameters::QEAveragedRefIndex(name.Data()),
														  WCSimDetectorParameters::PMTExposeHeight(name.Data()));
		fNumHitPMTs++;
	}
...
{
	if (digit < fNLikelihoodDigits)
		return (WCSimLikelihoodDigit *)fLikelihoodDigitArray->At(digit);
	else
	{
...
Int_t WCSimLikelihoodDigitArray::GetNDigits()
{
	return fNLikelihoodDigits;
}
Int_t WCSimLikelihoodDigitArray::GetNHits()
{
	return fNumHitPMTs;
}

Bool_t WCSimLikelihoodDigitArray::IsCylinder()
{
	return fGeomType == kCylinder;
}
Bool_t WCSimLikelihoodDigitArray::IsMailBox()
{
	return fGeomType == kMailBox;
}
WCSimLikelihoodDigitArray::GeomType_t WCSimLikelihoodDigitArray::GetGeomType()
{
	return fGeomType;
}
