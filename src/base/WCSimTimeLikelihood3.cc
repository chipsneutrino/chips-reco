...
double LogFinder::LogFirstArrival(const double x) const
{
	...
	return (log(fN * TMath::Sqrt2() / (fPredRMS * WCSimTimeLikelihood3::fSqrtPi)) - fN * WCSimTimeLikelihood3::fLog2 + (fN - 1) * log(TMath::Erfc((xmm) / (fPredRMS * TMath::Sqrt2()))) - (xmm) * (xmm) / (2 * fPredRMS * fPredRMS));
}
...
{
	double xmm = x - fPredMean;
	return 1.0 / (fPredRMS * TMath::Sqrt2() * WCSimTimeLikelihood3::fSqrtPi) * 2 * TMath::Exp(-xmm * xmm / (2 * fPredRMS * fPredRMS)) * ((1 - fN) / TMath::Erfc(xmm / (fPredRMS * TMath::Sqrt2()))) - (xmm) / (fPredRMS * fPredRMS);
}
...
WCSimTimeLikelihood3::WCSimTimeLikelihood3(WCSimLikelihoodDigitArray *myDigitArray,
										   WCSimEmissionProfileManager *myEmissionProfileManager) : fSpeedOfParticle(1.0 * TMath::C() * 1e-7)
{
...
void WCSimTimeLikelihood3::SetTracks(std::vector<WCSimLikelihoodTrackBase *> &myTracks)
{
	...
	if (IsGoodDigit(myDigit))
	{
		...
		if (whichCase == 0x11) // Predict and detect a hit - use PDF overlap
		{
			...
			if (timeLikelihood <= fMinimumLikelihood || timeLikelihood > 1)
			{
				...
				overlapFuncZoom.SetParameters(nPhotons, prediction.GetMean(), prediction.GetRMS(), myDigit->GetT(),
											  reso);
				ROOT::Math::WrappedTF1 f1(overlapFuncZoom);
				ROOT::Math::Integrator ig(f1, ROOT::Math::IntegrationOneDim::kADAPTIVESINGULAR);
				timeLikelihood = (1 - scatterFrac) * ig.Integral(prediction.GetMean() - 5 * prediction.GetRMS(),
		...
		{
			std::cout << "Digit is " << iDigit << " and timeLikelihood is " << timeLikelihood << " because arrives at "
					  << myDigit->GetT() << " and predict " << prediction.GetMean() << " with width "
					  << prediction.GetRMS() << " and res " << reso << std::endl;
...
void WCSimTimeLikelihood3::SetParticleSpeed(WCSimLikelihoodTrackBase *myTrack)
{
	...
	if (WCSimParameters::Instance()->UseCustomParticleSpeed())
	{
		fSpeedOfParticle = WCSimParameters::Instance()->GetCustomParticleSpeed() * speedOfLightInCmPerNs;
	}
	else
	{
...
{
	double n = 1.0;
	if (WCSimParameters::Instance()->UseCustomSpeedOfLight())
	{
		n = 1.0 / WCSimParameters::Instance()->GetCustomSpeedOfLight();
	}
	else if (WCSimParameters::Instance()->UseFittedSpeedOfLight())
	{
		n = 1.0 / WCSimParameters::Instance()->GetFittedSpeedOfLight();
	}
	else
	{
...
TimePrediction WCSimTimeLikelihood3::PredictArrivalTime(WCSimLikelihoodTrackBase *myTrack,
														WCSimLikelihoodDigit *myDigit)
{
	...
	for (size_t iEnergy = 0; iEnergy < fSVec.size(); ++iEnergy)
	{
		...
		// ============================================================================
		for (std::vector<StepParameters>::iterator stepItr = stepParameterVec.begin();
			 stepItr != stepParameterVec.end(); ++stepItr)
		{
			...
			if (prob > 0)
			{
				...
				t = myTrack->GetT() + fSVec[iEnergy]->GetBinCenter(stepItr->GetSBin()) / fSpeedOfParticle + n * stepItr->GetMagToPMT() / speedOfLightInCmPerNs;

				// Propagate a 3% uncertainty on the muon and photon speeds
				// to allow for a bit of scattering/wavelength dependence of n
				uncertaintySquared = (0.02 * particleDistance / fSpeedOfParticle) * (0.02 * particleDistance / fSpeedOfParticle) + (0.02 * photonDistance * n / speedOfLightInCmPerNs) * (0.02 * photonDistance * n / speedOfLightInCmPerNs);
			}
			else
			{
				prob = 0;
...
double WCSimTimeLikelihood3::FindOverlap(const double n, const double predMean, const double predRMS, const double t,
										 const double reso)
{
	...
	for (size_t interval = 0; interval < numCrossings - 1; ++interval)
	{
		...
		{
			double intervalInt = IntegrateMeanTimeToMeanFirstTime(max, predMean, predRMS, n) - IntegrateMeanTimeToMeanFirstTime(min, predMean, predRMS, n);
			integral += intervalInt;
 ...
 */
double WCSimTimeLikelihood3::FindCrossing(const double n, const double predMean, const double predRMS, const double t,
										  const double reso)
{
 ...
 * @return The x-value at which the two PDFs are equal
 */
std::vector<double> WCSimTimeLikelihood3::FindCrossings(const double n, const double predMean, const double predRMS,
														const double t, const double reso)
{
	...
	if (TMath::Finite(atMax) && atMax * lastEdge < 0 && max > x)
	{
		windowEdges.push_back(std::make_pair(x, max));
	}
 ...
 */
double WCSimTimeLikelihood3::ApproximateIntegral(const double n, const double predMean, const double predRMS,
												 const double t, const double reso)
{
 ...
 */
double WCSimTimeLikelihood3::ConvertMeanTimeToMeanFirstTime(const double &t, const double &mean, const double &sigma,
															const double &nPhotons)
{
	double prob = 0.0;
	if (nPhotons == 0 || sigma == 0)
	{
		return 0;
	}
	if (nPhotons < 25)
	{
		prob = nPhotons * TMath::Sqrt2() / (pow(2, nPhotons) * sigma * WCSimTimeLikelihood3::fSqrtPi) * pow(1.0 - WCSimFastMath::erf((t - mean) / (TMath::Sqrt2() * sigma)), nPhotons - 1) * TMath::Exp(-(t - mean) * (t - mean) / (2 * sigma * sigma));
	}
	else // Use logarithms to take care of the two large n powers almost cancelling
	{
		double logL = log(nPhotons * TMath::Sqrt2() / (sigma * WCSimTimeLikelihood3::fSqrtPi)) - nPhotons * WCSimTimeLikelihood3::fLog2 + (nPhotons - 1) * log(1.0 - WCSimFastMath::erf((t - mean) / (2 * sigma * sigma))) - (t - mean) * (t - mean) / (2 * sigma * sigma);
		prob = TMath::Exp(logL);
	}
	return prob;
}

double WCSimTimeLikelihood3::IntegrateGaussian(const double mean, const double sigma, const double from,
											   const double to)
{
...
double WCSimTimeLikelihood3::IntegrateGaussianToInfinity(const double mean, const double sigma, const double from)
{
	if (from > mean)
	{
 ...
 */
double WCSimTimeLikelihood3::IntegrateMeanTimeToMeanFirstTime(const double &t, const double &mean, const double &sigma,
															  const double &nPhotons)
{
	double integral(0.0);
	if (nPhotons <= 25)
...
double WCSimTimeLikelihood3::MinimumOfArrivalAndResolution(const double &t, const double &meanArr, const double rmsArr,
														   const double pmtTime, const double &pmtRes, const double &nPhotons)
{
	...
	if (TMath::IsNaN(arrivalPDF) || TMath::IsNaN(resolutionPDF))
	{
		...
		std::cout << "t = " << t << " and array mean = " << meanArr << " and array RMS = " << rmsArr
				  << " and PMT hit at " << pmtTime << " and PMT res = " << pmtRes << " and nPhotons = " << nPhotons
				  << std::endl;
		assert(0);
	}
...
double WCSimTimeLikelihood3::GetScatteredTimeLikelihood(WCSimLikelihoodDigit *myDigit)
{
	...
	{
		duration = 20;
	}								  // Put a floor of 20ns on the event length
	...
	if (prob < fMinimumLikelihood)
	{
		prob = fMinimumLikelihood;
	}
	return prob;
}
