 ...
 */
	WCSimDigitizerLikelihood::WCSimDigitizerLikelihood() : fType(kUnknown), fSK1peFile(NULL), fSK1peHist(NULL), fEfficiency(1.0), fPMTSimFile(NULL), fPMTSimHist(NULL), fTOTFile(
																																											NULL),
														   fTOTNikhefHist(NULL), fTOTMadisonHist(NULL)
{
...
Double_t WCSimDigitizerLikelihood::GetMinus2LnL(const Double_t &undigi, const Double_t &digi, std::string PMTName)
{
	...
	default:
	{
		...
		assert(
			fType == WCSimDigitizerLikelihood::kSK1pe || fType == WCSimDigitizerLikelihood::kPoisson || fType == WCSimDigitizerLikelihood::kPMTSim || fType == WCSimDigitizerLikelihood::kTOT);
		break;
	}
	}
