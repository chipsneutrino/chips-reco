...
// Constructor
//============
WCSimHoughTransformArray::WCSimHoughTransformArray(Int_t coneAngleBins, Double_t coneAngleMin, Double_t coneAngleMax,
												   Int_t phiBins, Int_t cosThetaBins)
{
	this->BuildArray(coneAngleBins, coneAngleMin, coneAngleMax, phiBins, cosThetaBins);
}
...
//============================================================================================================================================
void WCSimHoughTransformArray::BuildArray(Int_t coneAngleBins, Double_t coneAngleMin, Double_t coneAngleMax,
										  Int_t phiBins, Int_t cosThetaBins)
{
...
void WCSimHoughTransformArray::FindPeak(std::vector<Double_t> &houghDirX, std::vector<Double_t> &houghDirY,
										std::vector<Double_t> &houghDirZ, std::vector<Double_t> &houghAngle, std::vector<Double_t> &houghHeight)
{
											 ...
											 std::vector<Double_t> &houghDirZ, std::vector<Double_t> &houghAngle, std::vector<Double_t> &houghHeight)
{
	...
	nfound = search->SearchHighRes((const Double_t ***)source2, (Double_t ***)dest, fHoughX, fHoughY, fConeAngleBins, 3, 35, kTRUE, 2, kFALSE, 5);
	if (nfound > 0)
	{
		...
		PosX = (Float_t *)search->GetPositionX();
		PosY = (Float_t *)search->GetPositionY();
		PosAngle = (Float_t *)search->GetPositionZ();
											 ...
											 std::vector<Double_t> &houghDirZ, std::vector<Double_t> &houghAngle, std::vector<Double_t> &houghHeight)
{
	...
	Int_t nfound;
	std::cout << "Running peak finder" << std::endl;
	nfound = search->SearchHighRes((Double_t **)source, (Double_t **)dest, fHoughX, fHoughY, 4.0, 20, kTRUE, 5, kFALSE, 5);
	if (nfound > 0)
	{
		std::cout << "Found " << nfound << " peaks" << std::endl;
												   ...
												   std::vector<Double_t> &houghAngle, std::vector<Double_t> &houghHeight)
{
	...
	TH1D *houghProjection1D = new TH1D("houghProjection1D", ";cos#theta;height", houghSpace->GetYaxis()->GetNbins(),
									   houghSpace->GetYaxis()->GetXmin(), houghSpace->GetYaxis()->GetXmax());
	...
	for (unsigned int v = 0; v < peakListToSort.size(); ++v)
	{
		...
		if (peakListToSort[v].first < (maxHeight / 2.0))
			continue;
		...
		double dirY = sintheta * sin(phiradians);
		double dirZ = costheta;
		std::cout << "Phiradians = " << phiradians << "   costheta = " << costheta << "    (" << dirX << ", " << dirY
				  << ", " << dirZ << ") " << std::endl;
