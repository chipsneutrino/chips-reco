...
Double_t WCSimLikelihoodFitter::WrapFunc(const Double_t *x)
{
	...
	for (UInt_t iTrack = 0; iTrack < nTracks; ++iTrack)
	{
		...
		extraPars[FitterParameterType::kConversionDistance] =
			x[fFitterTrackParMap.GetIndex(trackParConversionDistance)];
		...
		WCSimLikelihoodTrackBase *track = WCSimLikelihoodTrackFactory::MakeTrack(
			fFitterTrackParMap.GetTrackType(iTrack), x[fFitterTrackParMap.GetIndex(trackParX)],
			x[fFitterTrackParMap.GetIndex(trackParY)], x[fFitterTrackParMap.GetIndex(trackParZ)],
			x[fFitterTrackParMap.GetIndex(trackParT)], x[fFitterTrackParMap.GetIndex(trackParTh)],
			x[fFitterTrackParMap.GetIndex(trackParPhi)], energy, extraPars);
	...
	tracksToFit.clear();
	std::cout << "Function evaluated " << ++fCalls << " times for event " << fEvent << " ... "
			  << " -2Ln(L) = "
			  << minus2LnL << std::endl
			  << std::endl;
	return minus2LnL;
...
double WCSimLikelihoodFitter::WrapFuncAlongTrack(const Double_t *x)
{
	...
	TVector3 vertex(fFitterTrackParMap.GetCurrentValue(trackParX), fFitterTrackParMap.GetCurrentValue(trackParY),
	...
	for (unsigned int i = 0; i < nTracks; ++i)
	{
		...
		directionTmp.SetMagThetaPhi(1.0, fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirTh),
	...
	for (unsigned int i = 0; i < nTracks; ++i)
	{
		WCSimLikelihoodTrackBase *track = WCSimLikelihoodTrackFactory::MakeTrack(fFitterTrackParMap.GetTrackType(i),
																				 newVertex.X(), newVertex.Y(), newVertex.Z(), time,
																				 fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirTh),
																				 fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirPhi), energies.at(i));
		tracksToFit.push_back(track);
	}
...
double WCSimLikelihoodFitter::WrapFuncPiZero(const Double_t *x)
{
	...
	WCSimLikelihoodTrackBase *track = WCSimLikelihoodTrackFactory::MakeTrack(trackType,
																			 x[fFitterTrackParMap.GetIndex(trackParX)], x[fFitterTrackParMap.GetIndex(trackParY)],
																			 x[fFitterTrackParMap.GetIndex(trackParZ)], x[fFitterTrackParMap.GetIndex(trackParT)],
																			 x[fFitterTrackParMap.GetIndex(trackParTh)], x[fFitterTrackParMap.GetIndex(trackParPhi)],
																			 x[fFitterTrackParMap.GetIndex(trackParE)], extraPars);
	tracksToFit.push_back(track);
	...
	track = WCSimLikelihoodTrackFactory::MakeTrack(trackType, x[fFitterTrackParMap.GetIndex(trackParX)],
												   x[fFitterTrackParMap.GetIndex(trackParY)], x[fFitterTrackParMap.GetIndex(trackParZ)],
												   x[fFitterTrackParMap.GetIndex(trackParT)], x[fFitterTrackParMap.GetIndex(trackParTh)],
												   x[fFitterTrackParMap.GetIndex(trackParPhi)], track1Energy, extraPars2);
	tracksToFit.push_back(track);
...
void WCSimLikelihoodFitter::FitEventNumber(Int_t iEvent)
{
	...
	fIsFirstCall = true;
	SetEvent(iEvent);
	bool usingCharge = WCSimParameters::Instance()->UseCharge();
	bool usingTime = WCSimParameters::Instance()->UseTime();
	...
	if (fFitterConfig->GetMakeFits() && CanFitEvent())
	{
		...
		else
		{
			...
			if (usingTime)
			{
				WCSimParameters::Instance()->SetUseTimeOnly();
				FreeTime();
				FixVertex();
				FixDirection();
				FixEnergy();
				...
				if (usingCharge)
				{
					WCSimParameters::Instance()->SetUseChargeAndTime();
				}
			}
			else
			{
			...
			if (WCSimParameters::Instance()->EqualiseChargeAndTime() && usingCharge && usingTime)
			{
			...
			if (usingCharge && usingTime)
			{
				...
				WCSimParameters::Instance()->SetUseChargeOnly();
...
void WCSimLikelihoodFitter::PerformEnergyGridSearch(Double_t &best2LnL, std::vector<Double_t> &bestEnergies)
{
}

WCSimLikelihoodTrackBase *WCSimLikelihoodFitter::RescaleParams(Double_t x, Double_t y, Double_t z, Double_t t,
															   Double_t th, Double_t phi, Double_t E, TrackType::Type type)
{
...
void WCSimLikelihoodFitter::RunFits()
{
	...
	for (UInt_t iEvent = firstEvent; iEvent < firstEvent + numEventsToFit; ++iEvent)
	{
		...
		// If the fit was successful fill the outputTree...
...
void WCSimLikelihoodFitter::RunSurfaces()
{
	...
	std::vector<std::pair<std::pair<unsigned int, FitterParameterType::Type>,
		...
		all2DProfile =
			fFitterPlots->GetAll2DSurfaceKeys();
	for (std::vector<
			 std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					   std::pair<unsigned int, FitterParameterType::Type>>>::iterator itr2D = all2DProfile.begin();
...
void WCSimLikelihoodFitter::RunLikelihoodPlots()
{
	...
	for (unsigned int digit = 0; digit < predictedCharges.size(); digit++)
	{
		...
		if (measuredCharges[digit] > 0)
		{
			pred_measured_Hist->Fill(predictedCharges[digit] - measuredCharges[digit]);
		}
...
void WCSimLikelihoodFitter::FillTree()
{
	if (fOutputTree != NULL)
	{
		...
		fOutputTree->Fill(eventHead, truthInfo, recoSumm, pidInfo);
	}
	return;
}
...
Bool_t WCSimLikelihoodFitter::GetTrackEscapes(WCSimLikelihoodTrackBase *track) const
{
	...
	Double_t distanceToEdge = WCSimGeometry::Instance()->ForwardProjectionToEdge(track->GetX(), track->GetY(),
																				 track->GetZ(), track->GetDirX(), track->GetDirY(), track->GetDirZ());
...
void WCSimLikelihoodFitter::SeedEvent()
{
	...
	// Run the old Hough transform reconstruction
	...
	for (unsigned int e = 0; e < slicedEvents.size(); ++e)
	{
		...
		for (int r = 0; r < slicedEvents[e]->GetNRings(); ++r)
		{
			...
			tempPair = std::make_pair<WCSimRecoRing *, double>(slicedEvents[e]->GetRing(r),
															   slicedEvents[e]->GetVtxTime());
	...
	for (unsigned int iTrack = 0; iTrack < fFitterConfig->GetNumTracks(); ++iTrack)
	{
		...
		if (iTrack < ringVec.size())
		{
			...
			if (fFitterConfig->GetNumTracks() == 1)
			{
				...
				vtxDir.Print();
				std::cout << "So angle is " << ringDir.Angle(vtxDir) << " = "
						  << ringDir.Angle(vtxDir) * TMath::RadToDeg() << std::endl;
				if (ringDir.Angle(vtxDir) * TMath::RadToDeg() > 3)
				{
					std::cerr
						<< "Warning: Direction for track is > 3 degrees different between naive vertex and Hough transform"
						<< std::endl;
					std::cerr << "This often indicates that something has gone wrong" << std::endl;
					std::cerr << "Will use the vertex direction (" << vtxDir.X() << ", " << vtxDir.Y() << ", "
							  << vtxDir.Z() << ")"
							  << " instead of the Hough direction (" << dirX << ", " << dirY << ", "
							  << dirZ << ")" << std::endl;
					dirX = vtxDir.X();
					dirY = vtxDir.Y();
					dirZ = vtxDir.Z();
		...
		std::cout << "Track seed " << iTrack << ": " << seedX << ", " << seedY << ", " << seedZ << " :: " << dirX
				  << ", " << dirY << ", " << dirZ << ", " << seedTheta << ", " << seedPhi << std::endl
				  << "Ring angle = "
				  << ringVec[iTrack]->GetAngle() << std::endl;
		...
		fSeeds.push_back(
			WCSimLikelihoodTrackFactory::MakeTrack(fFitterConfig->GetTrackType(iTrack), seedX, seedY, seedZ, seedT,
												   seedTheta, seedPhi, fFitterTrackParMap.GetCurrentValue(iTrack, FitterParameterType::kEnergy)));
	}
	...
	if (fOutputTree != NULL && WCSimParameters::Instance()->SaveStageInfo())
	{
		SetFitInfoStage();
	}
}
...
void WCSimLikelihoodFitter::FitEnergyGridSearch()
{
	...
	{
		std::cerr << "Error: Performing a grid search with more than two tracks will take an extremely long time"
				  << std::endl;
...
double WCSimLikelihoodFitter::FitAndGetLikelihood(const char *minAlgorithm)
{
	...
	for (UInt_t i = 0; i < nPars; ++i)
	{
		...
		{
			isAnythingFree = true;
			std::cout << "Free: " << names[i] << "  Start: " << startVals[i] << "   Min: " << minVals[i] << "   Max: "
					  << maxVals[i] << std::endl;
	...
	{
		std::cout << j << "   " << names[j] << "   " << startVals[j] << "   " << minVals[j] << "   " << maxVals[j]
				  << std::endl;
	}
...
void WCSimLikelihoodFitter::FitPiZero(const char *minAlgorithm)
{
	...
	for (UInt_t j = 0; j < nPars; ++j)
	{
		...
		std::cout << j << "   " << names[j] << "   " << isFixedStr << "   " << startVals[j] << "   " << minVals[j]
				  << "   " << maxVals[j] << std::endl;
	}
		...
		if (i == trk1EnergyIndex && GetUsePiZeroMassConstraint())
		{
			...
			if (secondEnergy < fFitterTrackParMap.GetMinValue(1, FitterParameterType::kEnergy))
			{
				...
				std::cerr << "Warning: Second photon energy of " << secondEnergy << " less than the allowed minimum. "
						  << "Using " << fFitterTrackParMap.GetMinValue(1, FitterParameterType::kEnergy) << "instead"
						  << std::endl;
			}
			else if (secondEnergy > fFitterTrackParMap.GetMaxValue(1, FitterParameterType::kEnergy))
			{
				...
				std::cerr << "Warning: Second photon energy of " << secondEnergy
						  << " greater than the allowed minimum. "
						  << "Using "
						  << fFitterTrackParMap.GetMaxValue(1, FitterParameterType::kEnergy) << "instead" << std::endl;
			}
			else
			{
...
void WCSimLikelihoodFitter::UpdateBestFits()
{
	...
	for (UInt_t iTrack = 0; iTrack < fFitterConfig->GetNumTracks(); ++iTrack)
	{
		...
		extraPars[FitterParameterType::kConversionDistance] = fFitterTrackParMap.GetCurrentValue(
			trackParConversionDistance);

		std::cout << "Type is " << fFitterTrackParMap.GetTrackType(iTrack) << std::endl;
		WCSimLikelihoodTrackBase *track = WCSimLikelihoodTrackFactory::MakeTrack(
			fFitterTrackParMap.GetTrackType(iTrack), fFitterTrackParMap.GetCurrentValue(trackParX),
			fFitterTrackParMap.GetCurrentValue(trackParY), fFitterTrackParMap.GetCurrentValue(trackParZ),
			fFitterTrackParMap.GetCurrentValue(trackParT), fFitterTrackParMap.GetCurrentValue(trackParTh),
			fFitterTrackParMap.GetCurrentValue(trackParPhi), fFitterTrackParMap.GetCurrentValue(trackParE),
			extraPars);
...
void WCSimLikelihoodFitter::MetropolisHastingsAlongTrack(const int nTries)
{
	...
	if (nTracks > 1)
	{
		...
		{
			if (fFitterConfig->GetTrackIsJoinedWith(i, "kVtxX") != 0 || fFitterConfig->GetTrackIsJoinedWith(i, "kVtxY") != 0 || fFitterConfig->GetTrackIsJoinedWith(i, "kVtxZ") != 0)
			{
				sharedVertex = false;
			}
		}
	...
	if (nTracks == 1 || (nTracks > 1 && sharedVertex))
	{
		...
		{
			TVector3 direction;
			direction.SetMagThetaPhi(1.0, fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirTh),
									 fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirPhi));
		...
		{
			if (escapeDists[i] < 0 && escapeDists[i] > minVal)
			{
				minVal = escapeDists[i];
			}
			if (escapeDists[i] > 0 && escapeDists[i] < maxVal)
			{
				maxVal = escapeDists[i];
			}
		}
		if (maxVal > 500)
		{
		...
		double stepSize = sqrt(
			pow((stepSizes.X() * direction.X()), 2) + pow((stepSizes.Y() * direction.Y()), 2) + pow((stepSizes.Z() * direction.Z()), 2));
		stepSize = 50;
		...
		while (iTry < nTries)
		{
			...
			for (size_t iTrack = 0; iTrack < nTracks; ++iTrack)
			{
				if (!fixE.at(iTrack))
				{
					newParams.at(fFitterTrackParMap.GetIndex(iTrack, FitterParameterType::kEnergy)) = newEnergies.at(
						iTrack);
				}
			}
			...
			for (size_t iTrack = 0; iTrack < nTracks; ++iTrack)
			{
				...
				if (!fixE.at(iTrack))
				{
					...
					shiftedParams.push_back(
						newParams.at(fFitterTrackParMap.GetIndex(iTrack, FitterParameterType::kEnergy)));
				}
			}
			std::cout << "Shifted params: "
					  << " x = " << newDistance << "   time = " << timeShift
					  << "   so new time = " << newTime << std::endl;
		...
		for (size_t iTrack = 0; iTrack < nTracks; ++iTrack)
		{
			if (!fixE.at(iTrack))
			{
				fFitterTrackParMap.SetCurrentValue(fFitterTrackParMap.GetIndex(iTrack, FitterParameterType::kEnergy),
												   runningTotals.at(2 + iTrack));
			}
		}
		UpdateBestFits();
	}
...
void WCSimLikelihoodFitter::FitAlongTrack()
{
	...
	bool canMove = (!fFitterTrackParMap.GetIsFixed(0, FitterParameterType::kVtxX) && !fFitterTrackParMap.GetIsFixed(0, FitterParameterType::kVtxY) && !fFitterTrackParMap.GetIsFixed(0, FitterParameterType::kVtxZ));
	...
	for (unsigned int i = 0; i < nTracks; ++i)
	{
		...
		direction.SetMagThetaPhi(
			fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kEnergy) - particle->Mass() * 1000,
			fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirTh),
			fFitterTrackParMap.GetCurrentValue(i, FitterParameterType::kDirPhi));
	...
	if (fixT == false && WCSimParameters::Instance()->UseTime()) // Adjust the time
	{
...
Double_t WCSimLikelihoodFitter::GetPiZeroSecondTrackEnergy(const Double_t *x)
{
	...
	std::cout << "Track 0 energy = " << track0Energy << "   Track 1 energy = " << track1Energy << "   Inv. mass = "
			  << sqrt((2 * track0Energy * track1Energy) * (1 - cosTheta01)) << std::endl;
	return track1Energy;
}
...
void WCSimLikelihoodFitter::Make2DSurfaceAlongTrack()
{
	...
	TString title = TString::Format(
		"Event %d, surface near minimum;Distance along reco track (cm);Time shift (ns);-2LnL", fEvent);
	...
	for (int iS = 0; iS < nBinsS; ++iS)
	{
		...
		x[yIndex] = newVtx.Y();
		x[zIndex] = newVtx.Z();
		std::cout << "S = " << s << " so new vertex = (" << newVtx.X() << ", " << newVtx.Y() << ", " << newVtx.Z()
				  << ")" << std::endl;
...
Bool_t WCSimLikelihoodFitter::IsInsideAllowedRegion(const Int_t &iTrack, const Double_t &x, const Double_t &y,
													const Double_t &z)
{
	//std::cout << "\n*** CHECKING IF INSIDE ***\n" << std::endl;
	...
	bool isInside = ((WCSimGeometry::Instance()->InsideDetector(x, y, z)) && !(x < fFitterTrackParMap.GetMinValue(iTrack, FitterParameterType::kVtxX)) && !(x > fFitterTrackParMap.GetMaxValue(iTrack, FitterParameterType::kVtxX)) && !(y < fFitterTrackParMap.GetMinValue(iTrack, FitterParameterType::kVtxY)) && !(y > fFitterTrackParMap.GetMaxValue(iTrack, FitterParameterType::kVtxY)) && !(z < fFitterTrackParMap.GetMinValue(iTrack, FitterParameterType::kVtxZ)) && !(z > fFitterTrackParMap.GetMaxValue(iTrack, FitterParameterType::kVtxZ)));
	return isInside;
}

Bool_t WCSimLikelihoodFitter::IsOutsideAllowedRegion(const Int_t &iTrack, const Double_t &x, const Double_t &y,
													 const Double_t &z)
{
...
Double_t WCSimLikelihoodFitter::GetPenalty(const std::vector<WCSimLikelihoodTrackBase *> &tracksToFit)
{
	...
	while (tIt != tracksToFit.end())
	{
		...
		{
			double radius = tb->GetX() * tb->GetX() + tb->GetY() * tb->GetY() - WCSimGeometry::Instance()->GetCylRadius() * WCSimGeometry::Instance()->GetCylRadius();
			double height = tb->GetZ() * tb->GetZ() - 0.25 * WCSimGeometry::Instance()->GetCylLength() * WCSimGeometry::Instance()->GetCylLength(); // length = full length, I want half length
			if (radius > 0)
			{
				penalty += radius;
			}
		...
		{
			double x = tb->GetX() * tb->GetX() - WCSimGeometry::Instance()->GetMailBoxX() * WCSimGeometry::Instance()->GetMailBoxX();
			double y = tb->GetY() * tb->GetY() - WCSimGeometry::Instance()->GetMailBoxY() * WCSimGeometry::Instance()->GetMailBoxY();
			double z = tb->GetZ() * tb->GetZ() - WCSimGeometry::Instance()->GetMailBoxZ() * WCSimGeometry::Instance()->GetMailBoxZ();
			if (x > 0)
			{
				penalty += x;
			}
		...
		if (x > maxX)
		{
			penalty += (maxX - x) * (maxX - x);
		}
		if (x < minX)
		{
			penalty += (minX - x) * (minX - x);
		}
		...
		if (y > maxY)
		{
			penalty += (maxY - y) * (maxY - y);
		}
		if (y < minY)
		{
			penalty += (minY - y) * (minY - y);
		}
		...
		if (z > maxZ)
		{
			penalty += (maxZ - z) * (maxZ - z);
		}
		if (z < minZ)
		{
			penalty += (minZ - z) * (minZ - z);
		}
		...
		if (t > maxT)
		{
			penalty += (maxT - t) * (maxT - t);
		}
		if (t < minT)
		{
			penalty += (minT - t) * (minT - t);
		}
...
WCSimRecoSummary WCSimLikelihoodFitter::BuildRecoSummary()
{
	...
	std::sort(fBestFit.begin(), fBestFit.end(), WCSimLikelihoodTrackBase::EnergyGreaterThanOrEqualPtrs);
	...
	for (unsigned int iTrack = 0; iTrack < fBestFit.size(); ++iTrack)
	{
		...
		rs.AddVertex(10 * track->GetX(), 10 * track->GetY(), 10 * track->GetZ(), track->GetT()); // Convert cm to mm

		rs.AddPrimary(track->GetPDG(), track->GetE(), track->GetDir());
	}
...
WCSimHitComparison WCSimLikelihoodFitter::BuildHitComparison()
{
	...
	{
		WCSimHitPrediction bestFit(predictedCharges.at(i), predictedTimes.at(i), best2LnLs.at(i), charge2LnLs.at(i),
								   time2LnLs.at(i), hit2LnLs.at(i));

		WCSimHitPrediction correctFit(correctPredictedCharges.at(i), correctPredictedTimes.at(i), correct2LnLs.at(i),
									  correctCharge2LnLs.at(i), correctTime2LnLs.at(i), correctHit2LnLs.at(i));
...
PidInfo WCSimLikelihoodFitter::BuildPidInfo()
{
	...
	pidInfo.SetHitInfo(veto, numVetoDigits, hits, hitsUpstream, hitsInBottom, hitsInTop, hitsAboveMid, charge,
					   chargeUpstream, chargeInBottom, chargeInTop, chargeAboveMid);

	float totalLikelihood = fMinimumTimeComponent + fMinimumChargeComponent + fMinimumHitComponent - fMinimumCutoffComponent;
	pidInfo.SetLikelihoods(totalLikelihood, fMinimumHitComponent, fMinimumChargeComponent, fMinimumTimeComponent);
	...
	double stoppingDistance = fTotalLikelihood->GetEmissionProfileManager()->GetStoppingDistance(fBestFit.at(0));
	...
	for (int iPMT = 0; iPMT < nPMTs; ++iPMT)
	{
		...
		{
			WCSimGeometry::RingRegion_t thisPMT = WCSimGeometry::Instance()->GetPMTRingRegion(
				fBestFit[iTrack]->GetVtx(), fBestFit[iTrack]->GetDir(), pmtPos, fBestFit[iTrack]->GetType(),
			...
			else if (thisPMT == WCSimGeometry::kInsideHole && region == WCSimGeometry::kOutside)
			{
				region = thisPMT;
			}
		}
...
void WCSimLikelihoodFitter::SetFitInfoStage()
{
	...
	if (fOutputTree != NULL)
	{
		...
		fTotalLikelihood->SetTracks(fBestFit);
		fMinimum = fTotalLikelihood->Calc2LnL();
		std::vector<double> stagePredictedCharges = fTotalLikelihood->GetPredictedChargeVector();
		std::vector<double> stagePredictedTimes = fTotalLikelihood->GetPredictedTimeVector();
		std::vector<double> stageHit2LnLs = fTotalLikelihood->GetHit2LnLVector();
		std::vector<double> stageCharge2LnLs = fTotalLikelihood->GetCharge2LnLVector();
		std::vector<double> stageTime2LnLs = fTotalLikelihood->GetTime2LnLVector();
		std::vector<double> stage2LnLs = fTotalLikelihood->GetTotal2LnLVector();
		...
		for (int i = 0; i < fLikelihoodDigitArray->GetNDigits(); ++i)
		{
			WCSimHitPrediction stageFit(stagePredictedCharges.at(i), stagePredictedTimes.at(i), stage2LnLs.at(i),
										stageCharge2LnLs.at(i), stageTime2LnLs.at(i), stageHit2LnLs.at(i));
			hitPredictions.push_back(stageFit);
		}

		std::vector<double> stageMinus2LnLs;
		stageMinus2LnLs.push_back(fMinimum);
