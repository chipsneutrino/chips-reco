...
void WCSimEmissionProfileManager::RemoveFromRecentEnergies(const TrackType::Type &type, const double &energy)
{
	...
	std::vector<double>::iterator energySearch = std::find((orderItr->second).begin(), (orderItr->second).end(),
														   energy);
...
void WCSimEmissionProfileManager::SetNumTracksToCache(const unsigned int numTracks)
{
	...
	{
		std::cerr << "Error: trying to cache " << numTracks
				  << " tracks - you need to cache at least four for the time likelihood" << std::endl;
		fNumTracksToCache = 4;
	}
...
std::vector<double> WCSimEmissionProfileManager::GetFourNearestEnergies(WCSimLikelihoodTrackBase *myTrack)
{
	return GetFourNearestEnergies(myTrack->GetType(), myTrack->GetE());
}

std::vector<double> WCSimEmissionProfileManager::GetFourNearestEnergies(const TrackType::Type &type,
																		const double &energy)
{
...
std::vector<TH1F *> WCSimEmissionProfileManager::GetFourNearestSForTimeHists(WCSimLikelihoodTrackBase *myTrack)
{
...
std::vector<TH2F *> WCSimEmissionProfileManager::GetFourNearestSCosThetaForTimeHists(WCSimLikelihoodTrackBase *myTrack)
{
	...
	if (fSCosThetaForTimeHists.size() == 0)
	{
		...
		for (size_t i = 0; i < energies.size(); ++i)
		{
			fSCosThetaForTimeHists[i] =
				(TH2F *)GetEmissionProfile(myTrack->GetType(), energies[i])->GetSCosThetaForTime()->Clone();
...
std::vector<Double_t> WCSimEmissionProfileManager::GetRhoIntegrals(std::vector<Int_t> sPowers,
																   WCSimLikelihoodTrackBase *myTrack, Double_t startS, Double_t endS, Bool_t multiplyByWidth)
{
	return GetEmissionProfile(myTrack)->GetRhoIntegrals(myTrack, sPowers, startS, endS, multiplyByWidth);
}

std::vector<double> WCSimEmissionProfileManager::GetRhoGIntegrals(WCSimLikelihoodTrackBase *myTrack,
																  WCSimLikelihoodDigit *myDigit, std::vector<int> sPowers, double cutoffS, bool multiplyByWidth)
{
	return GetEmissionProfile(myTrack)->GetRhoGIntegrals(myTrack, myDigit, sPowers, cutoffS, multiplyByWidth);
}

bool WCSimEmissionProfileManager::EnergiesInSameBin(const TrackType::Type &type, const double energy1,
