...
void WCSimCosmicFitter::SeedEvent()
{
	...
	if (dedicatedCosmicSeed)
		std::cout << "COSMIC SEED SUCCESS with " << ringVec.size() << " rings found." << std::endl;

	// If we don't have two slices, rely on the veto to seed things.
	if (dedicatedCosmicSeed)
	{
		if ((fFitterTrackParMap.GetTrackType(0) == TrackType::MuonLike && fFitterTrackParMap.GetTrackType(1) == TrackType::ElectronLike) || (fFitterTrackParMap.GetTrackType(1) == TrackType::MuonLike && fFitterTrackParMap.GetTrackType(0) == TrackType::ElectronLike))
		{
	...
	else
	{
		...
		if (takeAction)
		{
			...
			for (unsigned int i = 0; i < 2; ++i)
			{
				...
				if (seedDir[i].Y() != 0.0)
				{
					seedPhi.push_back(TMath::ATan2(seedDir[i].Y(), seedDir[i].X()));
				} // Ensure range is -pi to pi
				else
				{
					seedPhi.push_back((seedDir[i].X() < 0.0) ? 0.5 * TMath::Pi() : -0.5 * TMath::Pi());
				}
			}
...
void WCSimCosmicFitter::FitAlongTrack()
{
	...
	{
		bool canMove = (!fFitterTrackParMap.GetIsFixed(i, FitterParameterType::kVtxX) && !fFitterTrackParMap.GetIsFixed(i, FitterParameterType::kVtxY) && !fFitterTrackParMap.GetIsFixed(i, FitterParameterType::kVtxZ));
		...
		bool fixT = (fFitterTrackParMap.GetIsFixed(i, FitterParameterType::kVtxT) || !WCSimParameters::Instance()->UseTime());
		 ...
		 */
		std::cout << startVertex.X() << "," << startVertex.Y() << "," << startVertex.Z() << " :: " << direction.X()
		...
		if (fixT == false && WCSimParameters::Instance()->UseTime())
		{
		...
		std::cout << "Started at " << startVertex.X() << ", " << startVertex.Y() << ", " << startVertex.Z()
				  << std::endl;
		...
		ftpm.SetCurrentValue(ftpm.GetIndex(i, FitterParameterType::kVtxT),
							 ftpm.GetCurrentValue(i, FitterParameterType::kVtxT) + finalDistance / speed + min->X()[2]);
...
double WCSimCosmicFitter::WrapFuncAlongSingleTrack(const Double_t *x)
{
	...
	direction.SetMagThetaPhi(1.0, fFitterTrackParMap.GetCurrentValue(trackParTh),
	...
	for (unsigned int i = 0; i < nTracks; ++i)
	{
		...
		{
			track = WCSimLikelihoodTrackFactory::MakeTrack(fFitterTrackParMap.GetTrackType(i), newVertex.X(),
														   newVertex.Y(), newVertex.Z(), time, fFitterTrackParMap.GetCurrentValue(trackParTh),
														   fFitterTrackParMap.GetCurrentValue(trackParPhi), x[1]);
		}
		else
		{
			// Keep the other tracks as they were
			track = WCSimLikelihoodTrackFactory::MakeTrack(fFitterTrackParMap.GetTrackType(i),
