...
void WCSimAnalysisConfig::Print() {
	...
	std::cout << "fCalculateIntegrals = " << fCalculateIntegrals << std::endl << "fTruncateIntegrals  = "
			<< fTruncateIntegrals << std::endl << "fConstrainExtent = " << fConstrainExtent << std::endl
			<< "fDigiType = " << fDigiType << std::endl << "fUseTransmission = " << fUseTransmission << std::endl
			<< "fUseAngularEfficiency = " << fUseAngularEfficiency << std::endl << "fUseGlassCathodeReflection = "
			<< fUseGlassCathodeReflection << std::endl << "fUseTime = " << fUseTime << std::endl << "fUseCharge = "
			<< fUseCharge << std::endl << "fUseScatteringTable = " << fUseScatteringTable << std::endl
			<< "fEqualiseChargeAndTime = " << fEqualiseChargeAndTime << std::endl << "fSaveSeedInfo = " << fSaveSeedInfo
			<< std::endl << "fSaveStageInfo = " << fSaveStageInfo << std::endl << "fSaveHitComparison = "
			<< fSaveHitComparison << std::endl;
	return;
}
...
WCSimAnalysisConfig* WCSimAnalysisConfig::Instance() {
	...
	if (fgConfig == NULL) {
		fgConfig = new WCSimAnalysisConfig();
	}
	assert (fgConfig);
	if (fgConfig) {
	}
