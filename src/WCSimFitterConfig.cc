...
WCSimFitterConfig::WCSimFitterConfig() :
		fNumTracks(1), fNumEventsToFit(0), fFirstEventToFit(0) {
	fIsPiZeroFit = 0;
	fIsCosmicFit = 0;
	fNumParameters = 0;
	fForcePiZeroMass = 0;
...
void WCSimFitterConfig::SetParameter(unsigned int numTrack, const char * name, double min, double max, double start,
		double step, bool isFixed) {
...
void WCSimFitterConfig::FixTrackParameter(int numTrack, const char* name, bool doIt) {
	FitterParameterType::Type type = FitterParameterType::FromName(name);
	WCSimFitterSingleTrackParameters * trackPars = fFitterParameters.GetTrackParameters(numTrack);
...
void WCSimFitterConfig::FreeTrackParameter(int numTrack, const char* name, bool doIt) {
	FitterParameterType::Type type = FitterParameterType::FromName(name);
	WCSimFitterSingleTrackParameters * trackPars = fFitterParameters.GetTrackParameters(numTrack);
...
void WCSimFitterConfig::SetParStart(int numTrack, const char* name, double start) {
...
void WCSimFitterConfig::SetParStep(int numTrack, const char* name, double step) {
...
void WCSimFitterConfig::SetParRange(int numTrack, const char* name, double min, double max) {
...
std::pair<double, double> WCSimFitterConfig::GetParRange(unsigned int numTrack, const char* name) {
...
void WCSimFitterConfig::SetNumEventsToFit(int numEvents) {
	...
	if (numEvents > maxNumEvents) {
		std::cerr << "Warning: Requested to fit " << numEvents << " events, but the file only contains " << maxNumEvents
				<< std::endl;
		...
		std::cerr << "Warning: Requested to fit " << numEvents << " starting at event " << fFirstEventToFit
				<< " but there are only " << maxNumEvents << " in total" << std::endl;
		...
		std::cerr << "Warning: Requested to fit 0 events.  Will default to fitting all " << maxNumEvents << " events"
				<< std::endl;
		fNumEventsToFit = maxNumEvents;
	} else {
...
void WCSimFitterConfig::SetJoinParametersTogether(unsigned int numTrack1, unsigned int numTrack2, const char* name) {
	FitterParameterType::Type type = FitterParameterType::FromName(name);
	fFitterParameters.JoinParametersTogether(numTrack1, numTrack2, type);
}

bool WCSimFitterConfig::GetJoinParametersTogether(unsigned int numTrack1, unsigned int numTrack2, const char* name) {
	FitterParameterType::Type type = FitterParameterType::FromName(name);
...
void WCSimFitterConfig::SetTrackType(unsigned int numTrack, const char* typeName) {
...
void WCSimFitterConfig::SetFirstEventToFit(unsigned int iEvt) {
	...
	if (iEvt > maxNumEvents) {
		std::cerr << "Warning: Requested to start fitting at event " << iEvt << ", but the file only contains "
				<< maxNumEvents << std::endl;
		...
		std::cerr << "         But there are only " << maxNumEvents << " - will start at " << iEvt
...
void WCSimFitterConfig::SetForcePiZeroMass(const bool& doIt) {
	if (!fIsPiZeroFit) {
		std::cerr << "Can't force the mass to that of a pi zero without setting WCSimFitterConfig::fIsPiZeroFit first"
				<< std::endl;
		assert (fIsPiZeroFit);
	}
