 ...
 */
WCSimLikelihoodFitter::WCSimLikelihoodFitter(WCSimFitterConfig * config) :
		fFitterTrackParMap(config) {
	fCalls = 0;
	fEvent = -999;
...
Double_t WCSimLikelihoodFitter::WrapFunc(const Double_t * x) {
	...
	tracksToFit.clear();
	std::cout << "Function evaluated " << ++fCalls << " times for event " << fEvent << " ... " << " -2Ln(L) = "
			<< minus2LnL << std::endl << std::endl;
	return minus2LnL;
}
...
double WCSimLikelihoodFitter::WrapFuncAlongTrack(const Double_t * x) {
	...
	double speed = WCSimLikelihoodTrackBase::GetPropagationSpeedFrac(fFitterTrackParMap.GetTrackType(0)) * TMath::C()
			/ 1e7;
...
void WCSimLikelihoodFitter::RunSurfaces() {
	...
	std::vector
			< std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> > > all2DProfile =
...
void WCSimLikelihoodFitter::SeedEvent() {
	...
	for (unsigned int iTrack = 0; iTrack < fFitterConfig->GetNumTracks(); ++iTrack) {
			...
			if (fFitterConfig->GetNumTracks() == 1) {
				...
				if (ringDir.Angle(vtxDir) * TMath::RadToDeg() > 3) {
					...
					std::cerr << "Will use the vertex direction (" << vtxDir.X() << ", " << vtxDir.Y() << ", "
							<< vtxDir.Z() << ")" << " instead of the Hough direction (" << dirX << ", " << dirY << ", "
							<< dirZ << ")" << std::endl;
					dirX = vtxDir.X();
					dirY = vtxDir.Y();
					dirZ = vtxDir.Z();
		...
		std::cout << "Track seed " << iTrack << ": " << seedX << ", " << seedY << ", " << seedZ << " :: " << dirX
				<< ", " << dirY << ", " << dirZ << ", " << seedTheta << ", " << seedPhi << std::endl << "Ring angle = "
		...
		if (i == trk1EnergyIndex && GetUsePiZeroMassConstraint()) {
			...
			if (secondEnergy < fFitterTrackParMap.GetMinValue(1, FitterParameterType::kEnergy)) {
				...
				std::cerr << "Warning: Second photon energy of " << secondEnergy
						<< " greater than the allowed minimum. " << "Using "
...
void WCSimLikelihoodFitter::MetropolisHastingsAlongTrack(const int nTries) {
	...
	if (nTracks == 1 || (nTracks > 1 && sharedVertex)) {
		...
		double stepSize = sqrt(
				pow((stepSizes.X() * direction.X()), 2) + pow((stepSizes.Y() * direction.Y()), 2)
						+ pow((stepSizes.Z() * direction.Z()), 2));
		stepSize = 50;
		...
		while (iTry < nTries) {
			...
			std::cout << "Shifted params: " << " x = " << newDistance << "   time = " << timeShift
...
void WCSimLikelihoodFitter::FitAlongTrack() {
	...
	const double speed = WCSimLikelihoodTrackBase::GetPropagationSpeedFrac(fFitterTrackParMap.GetTrackType(0))
			* TMath::C() / 1e7;
	newVertex.Print();
...
PidInfo WCSimLikelihoodFitter::BuildPidInfo() {
	...
	float totalLikelihood = fMinimumTimeComponent + fMinimumChargeComponent + fMinimumHitComponent
