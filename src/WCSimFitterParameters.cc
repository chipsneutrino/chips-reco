...
WCSimFitterParameter::WCSimFitterParameter() :
		fType(FitterParameterType::kUnknown), fIsFixed(0), fStart(0), fMin(0), fMax(0) {
}

WCSimFitterParameter::WCSimFitterParameter(FitterParameterType::Type type, bool isFixed, double start, double min,
		double max, double step) :
		fType(type), fIsFixed(isFixed), fStart(start), fMin(min), fMax(max) {
	if (step == 0) {
		fStep = (fMax - fMin) / 250.0;
	} else {
		fStep = step;
	}
	return;
...
void WCSimFitterSingleTrackParameters::SetDefaultParameters() {
	...
	fParameters.clear();
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kVtxX, parX));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kVtxY, parY));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kVtxZ, parZ));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kVtxT, parT));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kDirTh, parTh));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kDirPhi,
					parPhi));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(FitterParameterType::kEnergy, parE));
	fParameters.insert(
			std::map<FitterParameterType::Type, WCSimFitterParameter>::value_type(
					FitterParameterType::kConversionDistance, parConv));
...
WCSimFitterParameter WCSimFitterSingleTrackParameters::GetParameter(FitterParameterType::Type type) {
...
void WCSimFitterSingleTrackParameters::SetParameter(FitterParameterType::Type type, bool isFixed, double start,
...
WCSimFitterSingleTrackParameters * WCSimFitterParameters::GetTrackParameters(unsigned int trackNum) {
...
void WCSimFitterParameters::JoinParametersTogether(unsigned int track1, unsigned int track2,
	...
	std::map<std::pair<unsigned int, unsigned int>, std::vector<FitterParameterType::Type> >::iterator joinItr =
			fJoinedParams.find(tracks);
...
bool WCSimFitterParameters::GetJoinParametersTogether(unsigned int track1, unsigned int track2,
		FitterParameterType::Type type) {
	std::pair<unsigned int, unsigned int> tracks = std::make_pair(track1, track2);
	std::map<std::pair<unsigned int, unsigned int>, std::vector<FitterParameterType::Type> >::iterator joinItr =
			fJoinedParams.find(tracks);
...
bool WCSimFitterParameters::GetIsParameterJoined(unsigned int track, FitterParameterType::Type type) {
	for (unsigned int iTrack = 0; iTrack < track; ++iTrack) {
		std::pair<unsigned int, unsigned int> tracks = std::make_pair(iTrack, track);
		std::map<std::pair<unsigned int, unsigned int>, std::vector<FitterParameterType::Type> >::iterator joinItr =
				fJoinedParams.find(tracks);
