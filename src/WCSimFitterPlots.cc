...
WCSimFitterPlots::WCSimFitterPlots(const TString &saveFileName) :
		fSaveFile(NULL), fNumSurfaceBins(20) {
...
void WCSimFitterPlots::MakePlotsForEachEvent(WCSimFitterConfig* fitterConfig) {
	...
	for (; plotItr != fForEachEvent.end(); ++plotItr) {
		...
		for (unsigned int iTrack = 0; iTrack < fitterConfig->GetNumTracks(); ++iTrack) {
			...
			title.Form("Value of %s for track %d, for each of %d fitted tracks", parName, iTrack,
			...
			hist = new TH1D(name.Data(), title.Data(), numBins, fitterConfig->GetParMin(iTrack, parName),
...
void WCSimFitterPlots::Make2DSurface(WCSimFitterConfig* fitterConfig) {
	...
	std::map<
			std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> >, TH2D *>::iterator plotItr =
			fSurfaces2D.begin();
...
void WCSimFitterPlots::FillNtuple(WCSimFitterConfig* fitterConfig, std::vector<WCSimLikelihoodTrackBase*> bestFits) {
}
...
void WCSimFitterPlots::Get2DSurfaceBinCenters(
	...
	std::map<
			std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> >, TH2D*>::iterator mapItr;
...
std::vector<
		std::pair<std::pair<unsigned int, FitterParameterType::Type>, std::pair<unsigned int, FitterParameterType::Type> > > WCSimFitterPlots::GetAll2DSurfaceKeys() const {
	std::map<
			std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> >, TH2D*>::const_iterator itr2D =
			fSurfaces2D.begin();
	std::vector
			< std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> > > myVec;
...
void WCSimFitterPlots::SaveProfiles() {
	...
	std::map<
			std::pair<std::pair<unsigned int, FitterParameterType::Type>,
					std::pair<unsigned int, FitterParameterType::Type> >, TH2D*>::iterator surf2DItr =
			fSurfaces2D.begin();
