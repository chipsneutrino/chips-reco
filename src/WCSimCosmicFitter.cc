 ...
 */
WCSimCosmicFitter::WCSimCosmicFitter(WCSimFitterConfig * config) :
...
void WCSimCosmicFitter::FitAlongTrack() {
	...
	for (unsigned int i = 0; i < nTracks; ++i) {
		...
		bool fixT = (fFitterTrackParMap.GetIsFixed(i, FitterParameterType::kVtxT)
...
//    min->Minimize();
		...
		const double speed = WCSimLikelihoodTrackBase::GetPropagationSpeedFrac(fFitterTrackParMap.GetTrackType(i))
				* TMath::C() / 1e7;
		newVertex.Print();
