...
public:
  ...
  WCSimRecoClusteringUtil(std::vector<WCSimRecoDigit*> digitVec, double qCut, double tCut, double dCut, unsigned int minSize);
  ~WCSimRecoClusteringUtil();

  void SetDigitVector(std::vector<WCSimRecoDigit*> digitVec){fDigitVec = digitVec;};
  void SetChargeCut(double qCut){fChargeCut = qCut;};
  void SetTimeCut(double tCut){fTimeCut = tCut;};
  void SetDistanceCut(double dCut){fDistanceCut = dCut;};
  void SetMinSize(unsigned int minSize){fMinSliceSize = minSize;};

  void PerformClustering();

  std::vector<std::vector<WCSimRecoDigit*> > GetFullSlicedDigits(){return fFullSlicedDigits;};
