...
class WCSimPiZeroElectronAdjuster: public WCSimPiZeroSeeder {
	public:
		WCSimPiZeroElectronAdjuster(WCSimFitterConfig * config, WCSimLikelihoodTrackBase * singleElectron,
				const double &minus2LnL);
	...
	private:
		...
		std::vector<WCSimPiZeroSeed*> IterateOverFirstTrackPerturbations(const double &convDistTrack1,
				const double &convDistTrack2);
		std::vector<TVector3> GetFirstTrackDirectionsToTry();
		std::vector<WCSimPiZeroSeed*> GridSearchOverSecondTrackDirection(const double &convDistTrack1,
				const double &convDistTrack2, const TVector3 &track1Dir, int numTimesAlready);
		...
		std::pair<double, double> GetPiZeroPhotonEnergies(const TVector3 &track1Dir, const double &track1Energy,
				const TVector3 &track2Dir);
