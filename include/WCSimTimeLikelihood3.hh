...
public:
	/**
		 ...
		 */
	LogFinder(double n, double predMean, double predRMS, double t, double reso) : fN(n), fPredMean(predMean), fPredRMS(predRMS), fT(t), fReso(reso){};
...
{
public:
	StepParameters(double cosTheta = 0, int sBin = 0, double deltaCosTheta = 0, double magToPMT = 0) : fCosTheta(cosTheta), fSBin(sBin), fDeltaCosTheta(deltaCosTheta), fMagToPMT(magToPMT){};
	inline double GetCosTheta() const
	{
		return fCosTheta;
	}
	inline double GetDeltaCosTheta() const
	{
		return fDeltaCosTheta;
	}
	inline double GetMagToPMT() const
	{
		return fMagToPMT;
	}
	inline int GetSBin() const
	{
		return fSBin;
	}
	bool operator<(const StepParameters &other) const
	{
		return fCosTheta < other.fCosTheta;
	}
...
public:
	...
	WCSimTimeLikelihood3(WCSimLikelihoodDigitArray *myDigitArray,
...
private:
	...
	/**
		 ...
		 */
	double FindCrossing(const double n, const double predMean, const double predRMS, const double t,
						const double reso);
	...
	/**
		 ...
		 */
	std::vector<double> FindCrossings(const double n, const double predMean, const double predRMS, const double t,
									  const double reso);
	...
	/**
		 ...
		 */
	double FindOverlap(const double n, const double predMean, const double predRMS, const double t,
					   const double reso);
	...
	/**
		 ...
		 */
	double ApproximateIntegral(const double n, const double predMean, const double predRMS, const double t,
							   const double reso);
	...
	/**
		 ...
		 */
	double ConvertMeanTimeToMeanFirstTime(const double &x, const double &mean, const double &sigma,
	...
	double ConvertMeanTimeToMeanFirstTimeTF1(double *x, double *par)
	{
		return ConvertMeanTimeToMeanFirstTime(x, par);
	};
	...
	/**
		 ...
		 */
	double IntegrateMeanTimeToMeanFirstTime(const double &t, const double &mean, const double &sigma,
											const double &nPhotons);
	...
	/**
		 ...
		 */
	double MinimumOfArrivalAndResolution(const double &t, const double &meanArr, const double rmsArr,
