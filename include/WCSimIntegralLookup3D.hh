...
public:
	...
	/**
		 ...
		 */
	WCSimIntegralLookupHistArray(double EMin, double EMax, double R0Max, double R0Min, int R0Bins, double cosTh0Max,
								 double cosTh0Min, int cosTh0Bins);
	WCSimIntegralLookupHistArray &operator=(const WCSimIntegralLookupHistArray &rhs);
	virtual ~WCSimIntegralLookupHistArray();

	void SetBins(double R0Max, double R0Min, int R0Bins, double cosTh0Max, double cosTh0Min, int cosTh0Bins);

	TGraph *GetRhoInt() const
	{
		return fRhoInt;
	}
	TGraph *GetRhoSInt() const
	{
		return fRhoSInt;
	}
	TGraph *GetRhoSSInt() const
	{
		return fRhoSSInt;
	}
