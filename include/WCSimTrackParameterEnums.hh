...
class TrackType: public TObject {
	public:
		enum Type {
			Unknown = 0, ElectronLike = 1, MuonLike = 2, PhotonLike = 3
		};
		Type fType;
		TrackType() :
				fType(Unknown) {
...
class FitterParameterType: public TObject {
	public:
		...
		;
		FitterParameterType(const FitterParameterType &fpt) :
				fType(fpt.fType) {
		}
		;
		operator Type() const {
