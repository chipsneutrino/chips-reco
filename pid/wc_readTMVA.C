/////////////////////////////////////////////////////////////////////////////////////////
//  Class and main function to read TMVA Neural Network weights for PID classification //
...
void wc_readTMVA(const char * inputFileName_mu, const char * inputFileName_el, const char * outputFileName){
    ...
    // These are functions of variables in the original tree, so let's add them to the ouput one explicitly
    ...
    for(int iReader = 0; iReader < NUM_READERS; ++iReader)
    {
        ...
        (readers[iReader])->AddVariable("fracQOutsideRing_mu", &fracQOutsideRing_mu);
        ...
        (readers[iReader])->AddVariable("recoE_el/totalQ", &recoEOverQ_el);
        (readers[iReader])->AddVariable("recoE_mu/totalQ", &recoEOverQ_mu);
        (readers[iReader])->AddVariable("charge2LnL_el/nHits", &chargeLikelihoodHitRatio_el);
        (readers[iReader])->AddVariable("charge2LnL_mu/nHits", &chargeLikelihoodHitRatio_mu);

	TString weightsDirectory = TString::Format("%s%s", std::getenv("WCSIMANAHOME"), "/pid/");
        TString weightPrefix;
	if(iReader == 0){ weightPrefix = TString("weights/tmvaTest_nueCCQE_vs_numuCCQE"); }     // my v0
	else if(iReader == 1){ weightPrefix = TString("weights/tmvaTest_nueCCQE_vs_numuNC"); }  // my v0

        std::cout << "Here iReader = " << iReader << std::endl;
        TString weightFile = weightsDirectory + weightPrefix + "_MLP.weights.xml";
    ...
    outputTree->Write();
    outputFile->Close();
    std::cout << " *** MVA results saved in file: " << outputFileName << std::endl;
