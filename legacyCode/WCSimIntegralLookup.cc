...
WCSimIntegralLookup::WCSimIntegralLookup(TString fileName) {
	...
	if (rhoInt == 0x0) {
		throw std::runtime_error("Could not get rhoInt from file");
	}
	if (rhoSInt == 0x0) {
		throw std::runtime_error("Could not get rhoSInt from file");
	}
	if (rhoSSInt == 0x0) {
		throw std::runtime_error("Could not get rhoSSInt from file");
	}
	if (rhoGInt == 0x0) {
		throw std::runtime_error("Could not get rhoGInt from file");
	}
	if (rhoGSInt == 0x0) {
		throw std::runtime_error("Could not get rhoGSInt from file");
	}
	if (rhoGSSInt == 0x0) {
		throw std::runtime_error("Could not get rhoGSSInt from file");
	}
	if (cutoffS == 0x0) {
		throw std::runtime_error("Could not get cutoffS from file");
	}
...
double WCSimIntegralLookup::GetRhoGSSIntegral(const double &E, const double &s, const double &R0,
		const double &cosTh0) {
...
double WCSimIntegralLookup::GetIntegral4D(const double &E, const double &s, const double &R0, const double &cosTh0,
		THnSparseF * hist) {
	double cutoff = GetCutoffS(E);
