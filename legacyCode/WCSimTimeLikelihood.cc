...
Double_t WCSimTimeLikelihood::TimeLikelihood(Int_t trackIndex, Double_t correctedTime,
	...
	for (ibin = 0; ibin < fNumChargeCuts; ibin++) {
		if (charge >= fChargeCuts[ibin] && charge < fChargeCuts[ibin + 1])
			break;
	}
	double chargeBin[1];
	chargeBin[0] = ibin;
...
void WCSimTimeLikelihood::GetTrackParameters(Int_t trackIndex) {
	...
	for (int i = 0; i < 7; i++) {
		for (int j = 0; j < 5; j++) {
			fSecondaryParameters[i][j] = fEnergyParameterFunction->EvalPar(x, fTertiaryParameters[i][j]);
		}
	}
