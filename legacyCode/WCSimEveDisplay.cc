...
void WCSimEveDisplay::BuildEvePointSets() {
	...
	Int_t colours[fNumBins] = { colourCode0, colourCode1, colourCode2, colourCode3, colourCode4, colourCode5,
			colourCode6, colourCode7, colourCode8, colourCode9 };
...
void WCSimEveDisplay::BuildTrueTracks(WCSimTruthSummary *mySummary) {
	...
	if (mySummary->IsParticleGunEvent()) {
		...
		if (mySummary->GetBeamPDG() == 111) {
			...
			double mm_to_cm = 0.1;
			TVector3 vtx(mySummary->GetVertexX() * mm_to_cm, mySummary->GetVertexY() * mm_to_cm,
					mySummary->GetVertexZ() * mm_to_cm);
			TVector3 dir(mySummary->GetBeamDir().X(), mySummary->GetBeamDir().Y(), mySummary->GetBeamDir().Z());
			TVector3 end = vtx
					+ WCSimGeometry::Instance()->ForwardProjectionToEdge(vtx.X(), vtx.Y(), vtx.Z(), dir.X(), dir.Y(),
							dir.Z()) * dir;
		...
		for (size_t iTrack = 0; iTrack < mySummary->GetNPrimaries(); ++iTrack) {
			...
			for (unsigned int i = 0; i < mySummary->GetNPrimaries(); ++i) {
				if (abs(particle->PdgCode()) == 12 || abs(particle->PdgCode()) == 14) {
					...
					TVector3 end = vtx
							+ WCSimGeometry::Instance()->BackwardProjectionToEdge(vtx.X(), vtx.Y(), vtx.Z(), dir.X(),
									dir.Y(), dir.Z()) * dir;
					neutrinoTrack->AddLine(vtx.X(), vtx.Y(), vtx.Z(), end.X(), end.Y(), end.Z());
				}

				if ((particle->Charge() == 0 && particle->PdgCode() != 22)
						|| mySummary->GetPrimaryEnergy(iTrack) < 100) {
					continue;
				}
				...
				if ((particle->Charge() != 0 || particle->PdgCode() == 22)
					...
					if (beta > 3 / 4.) // Assume refractive index of 4/3 for the purposes of drawing
						...
						TVector3 dir = mySummary->GetPrimaryDir(iTrack);
						TVector3 end = vtx
								+ WCSimGeometry::Instance()->ForwardProjectionToEdge(vtx.X(), vtx.Y(), vtx.Z(), dir.X(),
										dir.Y(), dir.Z()) * dir;
