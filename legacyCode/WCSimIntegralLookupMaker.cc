...
WCSimIntegralLookupMaker::WCSimIntegralLookupMaker(TrackType::Type particle, int nR0Bins, double R0Min, double R0Max,
		int nCosTh0Bins, double cosTh0Min, double cosTh0Max) {
	std::cout << " *** WCSimIntegralLookupMaker for track type = " << TrackType::AsString(particle) << " *** "
			<< std::endl;
	fRhoInt = 0x0;
	fRhoSInt = 0x0;
	fRhoSSInt = 0x0;
	...
	SetBins(binningHist->GetXaxis()->GetNbins(), binningHist->GetXaxis()->GetXmin(), eMax,
			hRhoTemp->GetXaxis()->GetNbins(), hRhoTemp->GetXaxis()->GetXmin(), hRhoTemp->GetXaxis()->GetXmax(), nR0Bins,
			R0Min, R0Max, nCosTh0Bins, cosTh0Min, cosTh0Max);
}
...
void WCSimIntegralLookupMaker::SetBins(const int &nEBins, const double &eMin, const double &eMax, const int &nSBins,
		const double &sMin, const double &sMax, const int &nR0Bins, const double &R0Min, const double &R0Max,
	...
	fRhoGSInt = new THnSparseF("fRhoGSInt", "Integral of #rho(s) #times g(s, cos#theta) #times s", 4, binsG, minG,
			maxG);
	fRhoGSSInt = new THnSparseF("fRhoGSSInt", "Integral of #rho(s) #times g(s, cos#theta) #times s^{2}", 4, binsG, minG,
			maxG);
...
void WCSimIntegralLookupMaker::MakeRhoGTables() {
	...
	std::cout << "Fraction of bins = " << fIntegrals.GetRhoGInt()->GetSparseFractionBins() << " and of memory "
			<< fIntegrals.GetRhoGInt()->GetSparseFractionMem() << std::endl;
...
void WCSimIntegralLookupMaker::CheckBins(const int nBins, const double min, const double max) {
	if (nBins <= 0) {
		std::cerr
				<< "WCSimIntegralLookupMaker::CheckBins: You need to have a positive integer number of bins, but you have "
				<< nBins << std::endl;
		assert(nBins > 0);
	}

	if (min >= max) {
		std::cerr << "WCSimIntegralLookupMaker::CheckBins: Your minimum value of " << min
				<< " is not smaller than your maximum value of " << max << std::endl;
		assert(min < max);
	}
	...
	if (fmod(width, 1e-6) > 1e-15) {
		std::cerr
				<< "WCSimIntegralLookupMaker::CheckBins: You need to have bin widths that can be expressed by a terminating decimal"
				<< std::endl;
