...
WCSimDisplayViewer::WCSimDisplayViewer() :
		fRootStyle(0), fControlCanvas(0), buttonStart(0), buttonNext(0), buttonPrevious(0), buttonGoTo(0), buttonPrint(
				0), buttonQuit(0), fCounter(-1), fReco(0), fRecoEvent(0), fTrueEvent(0), fDisplayRecoClusters(0), fDisplayRecoEvent(
				0), fDisplayTrueEvent(0), fPulseHeightCut(0.0), fPrintGIF(0), fPrintEPS(0), fDisplayReady(0) {
	this->Initialize();
}

WCSimDisplayViewer::~WCSimDisplayViewer() {
	if (fControlCanvas)
		delete fControlCanvas;
	if (buttonStart)
		delete buttonStart;
	if (buttonNext)
		delete buttonNext;
...
void WCSimDisplayViewer::Initialize() {
	...
	if (!fControlCanvas) {
		fControlCanvas = new TCanvas("ControlCanvas", "ControlCanvas", 0, 0, 150, 600);
		fControlCanvas->cd();
...
void WCSimDisplayViewer::UseDisplay(const char* newdisplay) {
	...
	for (UInt_t n = 0; n < fDisplayNameList.size(); n++) {
		...
		if (strcmp(olddisplay, newdisplay) == 0)
			foundit = 1;
	}
...
void WCSimDisplayViewer::UpdateDisplay(Int_t entry) {
	...
	if (WCSimInterface::TouchEvent()) {
		...
		// run reconstruction
		if (fDisplayRecoClusters || fDisplayRecoEvent) {
...
void WCSimDisplayViewer::controlGoTo() {
	if (fDisplayReady) {
		...
		// go forwards
		if (!foundit && event > fEventNum) {
			tempCounter = oldCounter;
			tempEvent = fEventNum;
			while (tempCounter < WCSimInterface::GetNumEvents() - 1 && (tempEvent < 0 || tempEvent < event)
					&& !(tempEvent == event)) {
				tempCounter++;
		...
		if (!foundit && event < fEventNum) {
			tempCounter = oldCounter;
			tempEvent = fEventNum;
			while (tempCounter > 0 && (tempEvent < 0 || tempEvent > event) && !(tempEvent == event)) {
				tempCounter--;
				WCSimInterface::LoadEvent(tempCounter);
				if (WCSimInterface::TouchEvent()) {
					tempEvent = WCSimInterface::GetEventNumber();
