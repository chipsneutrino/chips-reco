...
WCSimDisplayAB::WCSimDisplayAB() :
		WCSimDisplay(), fGeoType(-1), fCylRadius(0.0), fCylLength(0.0), fCylDiagonal(0.0), fMailBoxX(0.0), fMailBoxY(
				0.0), fMailBoxZ(0.0), fMailBoxDiagonal(0.0), fScale(1.0), wcCanvas(0), wcDisplay(0), wcLegend(0), wcCylEdgeSide(
				0), wcCylEdgeTop(0), wcCylEdgeBottom(0), wcBoxEdgeSide(0), wcBoxEdgeTop(0), wcBoxEdgeBottom(0), wcBoxEdgeLine1(
				0), wcBoxEdgeLine2(0), wcBoxEdgeLine3(0), wcBoxEdgeLine4(0), wcTimeCanvas(0), wcTimeLegend(0), wcRecoLegend(
				0) {
...
void WCSimDisplayAB::Reset() {
	...
	if (wcCylEdgeSide)
		delete wcCylEdgeSide;
	wcCylEdgeSide = 0;
	if (wcCylEdgeTop)
		delete wcCylEdgeTop;
	wcCylEdgeTop = 0;
	if (wcCylEdgeBottom)
		delete wcCylEdgeBottom;
	wcCylEdgeBottom = 0;
	if (wcBoxEdgeSide)
		delete wcBoxEdgeSide;
	wcBoxEdgeSide = 0;
	if (wcBoxEdgeTop)
		delete wcBoxEdgeTop;
	wcBoxEdgeTop = 0;
	if (wcBoxEdgeBottom)
		delete wcBoxEdgeBottom;
	wcBoxEdgeBottom = 0;
	if (wcBoxEdgeLine1)
		delete wcBoxEdgeLine1;
	wcBoxEdgeLine1 = 0;
	if (wcBoxEdgeLine2)
		delete wcBoxEdgeLine2;
	wcBoxEdgeLine2 = 0;
	if (wcBoxEdgeLine3)
		delete wcBoxEdgeLine3;
	wcBoxEdgeLine3 = 0;
	if (wcBoxEdgeLine4)
		delete wcBoxEdgeLine4;
	wcBoxEdgeLine4 = 0;
...
void WCSimDisplayAB::BuildGeometry() {
	...
	if (fGeoType == WCSimGeometry::kCylinder) {
		...
		// make canvas
		wcCanvas = new TCanvas("WCSimDisplay", "WCSim Event Display", canvasU, canvasV);

		// make histogram
		wcDisplay = new TH2D("wcDisplay", "", binsU, -0.5 * fU * fScale, +0.5 * fU * fScale, binsV, -0.5 * fV * fScale,
				+0.5 * fV * fScale);
		...
		if (fMakeTimeDisplay) {
			wcTimeCanvas = new TCanvas("WCSimTimeDisplay", "WCSim Timing Display", canvasU, canvasV);
	...
	if (fGeoType == WCSimGeometry::kMailBox) {
		...
		// outline for top face of detector
		wcBoxEdgeTop = new TBox(-0.5 * fMailBoxY * fScale, +0.5 * fMailBoxZ * fScale, +0.5 * fMailBoxY * fScale,
				+(0.5 * fMailBoxZ + fMailBoxX) * fScale);
		wcBoxEdgeTop->SetFillStyle(0);
		wcBoxEdgeTop->SetLineColor(1);
		wcBoxEdgeTop->SetLineWidth(2);

		// outline for bottom face of detector
		wcBoxEdgeBottom = new TBox(-0.5 * fMailBoxY * fScale, -0.5 * fMailBoxZ * fScale, +0.5 * fMailBoxY * fScale,
				-(0.5 * fMailBoxZ + fMailBoxX) * fScale);
		...
		wcBoxEdgeLine2 = new TLine(-0.5 * fMailBoxY * fScale, -0.5 * fMailBoxZ * fScale, -0.5 * fMailBoxY * fScale,
				+0.5 * fMailBoxZ * fScale);
		wcBoxEdgeLine2->SetLineColor(1);
		wcBoxEdgeLine2->SetLineWidth(2);
		wcBoxEdgeLine2->SetLineStyle(7);

		wcBoxEdgeLine3 = new TLine(+0.5 * fMailBoxY * fScale, -0.5 * fMailBoxZ * fScale, +0.5 * fMailBoxY * fScale,
				+0.5 * fMailBoxZ * fScale);
...
void WCSimDisplayAB::DrawNewDisplay() {
	...
	if (wcCanvas) {
		...
		wcLegend->Draw();
		wcRecoLegend->Draw();
		wcTitleLatex.DrawLatex(titleU * fScale, titleV * fScale, wcTitleString.Data());
		wcCanvas->Update();
	}
...
void WCSimDisplayAB::DrawDisplay(WCSimRecoEvent* myRecoEvent) {
	...
	for (Int_t nDigit = 0; nDigit < myRecoEvent->GetNDigits(); nDigit++) {
		...
		// convert XYZ to UV
		WCSimGeometry::Instance()->XYZtoUV(region, x, y, z, u, v);
...
void WCSimDisplayAB::DrawTrueEvent(WCSimRootEvent* myEvent) {
	...
	if (myTrueEvent == 0 || myTrueEvent->GetNTracks() == 0)
		return;
	...
	// draw true vertex
	this->DrawVertex(evt_x0, evt_y0, evt_z0, evt_px, evt_py, evt_pz, evt_colourcode);
	...
	for (Int_t nTrack = 0; nTrack < myTrueEvent->GetNTracks(); nTrack++) {
		...
		// draw true ring
		this->DrawRing(x0, y0, z0, px, py, pz, angle, colourcode);
	}
...
void WCSimDisplayAB::DrawRecoEvent(WCSimRecoEvent* myRecoEvent) {
	// Check for Event
	// ===============
	if (myRecoEvent == 0 || myRecoEvent->FoundVertex() == 0 || myRecoEvent->FoundDirection() == 0)
		return;
...
void WCSimDisplayAB::DrawVertex(Double_t vx, Double_t vy, Double_t vz, Double_t px, Double_t py, Double_t pz,
		Int_t colourcode) {
	...
	WCSimGeometry::Instance()->ProjectToFarEdge(vx, vy, vz, px, py, pz, x, y, z, region);
...
void WCSimDisplayAB::DrawRing(Double_t vx, Double_t vy, Double_t vz, Double_t px, Double_t py, Double_t pz,
	...
	WCSimGeometry::Instance()->ProjectToFarEdge(vx, vy, vz, px, py, pz, xproj, yproj, zproj, region);
	...
	for (Int_t n = 0; n < n_ring; n++) {
		...
		WCSimGeometry::FindCircle(xproj, yproj, zproj, vx, vy, vz, cone_angle, n * delta_omega, xring, yring, zring, nx,
				ny, nz, radius);

		WCSimGeometry::Instance()->ProjectToFarEdge(vx, vy, vz, nx, ny, nz, x, y, z, region);
...
void WCSimDisplayAB::PrintDisplay() {
	// print GIF
	if (fPrintGIF)
		this->PrintDisplayGIF();

	// print EPS
	if (fPrintEPS)
		this->PrintDisplayEPS();
}
...
Int_t WCSimDisplayAB::TIMEtoCOL(Double_t T) {
	...
	if (T - fBaseT < -10.0)
		colourCode = 98;
	if (T - fBaseT >= -10.0 && T - fBaseT < 10.0)
		colourCode = 98;
	if (T - fBaseT >= 10.0 && T - fBaseT < 30.0)
		colourCode = 94;
	if (T - fBaseT >= 30.0 && T - fBaseT < 50.0)
		colourCode = 89;
	if (T - fBaseT >= 50.0 && T - fBaseT < 100.0)
		colourCode = 85;
	if (T - fBaseT >= 100.0 && T - fBaseT < 150.0)
		colourCode = 80;
	if (T - fBaseT >= 150.0 && T - fBaseT < 200.0)
		colourCode = 75;
	if (T - fBaseT >= 200.0 && T - fBaseT < 300.0)
		colourCode = 70;
	if (T - fBaseT >= 300.0 && T - fBaseT < 450.0)
		colourCode = 65;
	if (T - fBaseT >= 450.0 && T - fBaseT < 650.0)
		colourCode = 55;
	if (T - fBaseT >= 650.0)
		colourCode = 55;
