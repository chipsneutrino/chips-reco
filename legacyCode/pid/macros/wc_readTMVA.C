...
//  (NueCC vs NumuCC) and (NueCC vs NC) using  WCSimAnalyiss output Tree               //
...
void wc_readTMVA(const char * inputFileName_mu, const char * inputFileName_el, const char * baseOutputName, const char * PIDDir){
    ...
    for(int f=0; f<muFileVec.size(); f++)
    {
		...
		if(inputTree_mu != NULL && inputTree_el != NULL)
		{
			...
			if(numEvents_mu == numEvents_el)
			{
			    ...
			    // Readers for the nue CCQE vs. numu CCQE MVAs and the
			    // nue CCQE vs NC trees
			    TMVA::Reader * readers[NUM_READERS];
			    readers[0] = new TMVA::Reader("!Color:!Silent");
			    readers[1] = new TMVA::Reader("!Color:!Silent");


			    // Setup the readers. Add all the tree variables to the MVA and load the weights
			    for(int iReader = 0; iReader < NUM_READERS; ++iReader)
			    {
			    	if(iReader == 0){
			    		// nue CCQE vs. numu variables
			    		(readers[iReader])->AddVariable("nHits",&nHits);
			    		(readers[iReader])->AddVariable("deltaCharge2LnL",&deltaCharge2LnL);
			    		(readers[iReader])->AddVariable("deltaTime2LnL",&deltaTime2LnL);
			    		(readers[iReader])->AddVariable("deltaCharge2LnLOverNHits",&deltaCharge2LnLOverNHits);
			    		(readers[iReader])->AddVariable("fracHitsOutsideRing_mu",&fracHitsOutsideRing_mu);
			    		(readers[iReader])->AddVariable("fracHitsInRing_mu",&fracHitsInRing_mu);
			    		(readers[iReader])->AddVariable("fracHitsInRingHole_mu",&fracHitsInRingHole_mu);
			    		(readers[iReader])->AddVariable("fracHitsOutsideRing_el",&fracHitsOutsideRing_el);
			    		(readers[iReader])->AddVariable("fracHitsInRing_el",&fracHitsInRing_el);
			    		(readers[iReader])->AddVariable("fracHitsInRingHole_el",&fracHitsInRingHole_el);
			    		(readers[iReader])->AddVariable("fracPredQOutsideRing_mu",&fracPredQOutsideRing_mu);
			    		(readers[iReader])->AddVariable("fracPredQOutsideRing_el",&fracPredQOutsideRing_el);
			    		(readers[iReader])->AddVariable("predictedChargeOverTotalCharge_mu",&predictedChargeOverTotalCharge_mu);
			    		(readers[iReader])->AddVariable("predictedChargeOverTotalCharge_el",&predictedChargeOverTotalCharge_el);
			    		(readers[iReader])->AddVariable("recoEOverQ_mu",&recoEOverQ_mu);
			    		(readers[iReader])->AddVariable("recoEOverQ_el",&recoEOverQ_el);

			    		(readers[iReader])->AddVariable("nRings",&nRings);
			    		(readers[iReader])->AddVariable("firstRingHeight",&firstRingHeight);
			    		(readers[iReader])->AddVariable("lastRingHeight",&lastRingHeight);
			    	}
			    	if(iReader == 1){
			    		// nue CCQE vs NC variables
			    		(readers[iReader])->AddVariable("nHits",&nHits);
			    		(readers[iReader])->AddVariable("deltaCharge2LnL",&deltaCharge2LnL);
			    		(readers[iReader])->AddVariable("deltaTime2LnL",&deltaTime2LnL);
			    		(readers[iReader])->AddVariable("deltaCharge2LnLOverNHits",&deltaCharge2LnLOverNHits);
			    		...
			    		(readers[iReader])->AddVariable("recoEOverQ_mu",&recoEOverQ_mu);
			    		(readers[iReader])->AddVariable("recoEOverQ_el",&recoEOverQ_el);
			    		(readers[iReader])->AddVariable("dirX_el",&dirX_el);
			    		(readers[iReader])->AddVariable("fracHitsDownstream",&fracHitsDownstream);
			    	...
			    	// Set the weight file according to type of PID
			    	std::string weightsDirectory(PIDDir);
			        TString weightPrefix;
			        if(iReader == 0){ weightPrefix = TString("/weights/tmva_nueCCQE_vs_numuCCQE_MLP.weights.xml"); }
			        else if(iReader == 1){ weightPrefix = TString("/weights/tmva_nueCCQE_vs_numuNC_MLP.weights.xml"); }
			        TString weightFile = TString(weightsDirectory) + weightPrefix;

			        std::cout << "Here iReader = " << iReader << std::endl;
			        std::cout << "Weights in " << weightFile << std::endl;
			        readers[iReader]->BookMVA("MLP", weightFile);
			    }

			    // Create the ANN value histograms
			    TH1F * histANNNueCCQEvsNumuCCQE = new TH1F("histANNNueCCQEvsNumuCCQE","#nu_{e} vs #nu_{#mu} CCQE discriminator;ANN variable;Events", 200, -2, 2);
			    TH1F * histANNNueCCQEvsNumuNC = new TH1F("histANNNueCCQEvsNumuNC","#nu_{e} vs #nu_{#mu} NC discriminator;ANN variable;Events", 200, -2, 2);
			    ...
			    for(Int_t iEvent = 0; iEvent < numEvents; ++iEvent)
			    {
					...
					dirX_el = treeEntry->GetDirX_el();
					fracHitsDownstream = treeEntry->GetFracHitsDownstream();
					...
					annNueCCQEvsNumuCCQE = readers[0]->EvaluateMVA("MLP");
					annNueCCQEvsNC       = readers[1]->EvaluateMVA("MLP");

					// Fill the ANN value histograms and fill the PIDTree
					histANNNueCCQEvsNumuCCQE->Fill(annNueCCQEvsNumuCCQE);
					histANNNueCCQEvsNumuNC->Fill(annNueCCQEvsNC);

					treeEntry->SetNueVsNumu(annNueCCQEvsNumuCCQE);
					treeEntry->SetNueVsNC(annNueCCQEvsNC);
			    ...
			    histANNNueCCQEvsNumuCCQE->Write();
			    histANNNueCCQEvsNumuNC->Write();
			    outputTree->Write();
			    outputFile->Close();
			}
			else{ numSkipped += 1; }
    ...
    std::cout << "Read and Combined " << (muFileVec.size() - numSkipped) << " files." << std::endl;
